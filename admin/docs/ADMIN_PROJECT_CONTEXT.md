...
- 视频管理：维护 YouTube / Bilibili / b23.tv 视频，服务端识别链接、抓取标题、简介、作者、发布时间、封面和规范化 `embed_url`，支持草稿、发布、隐藏、排序、置顶、删除和刷新元数据。置顶独立优先，未置顶视频按排序值从大到小显示，新建默认最大排序 +10；后台编辑区只展示检查用小播放器，避免 iframe 预览占满页面。
- 视频分类管理：维护视频区分类标签，支持 slug、中文名、English、日本語、排序和启用状态；分类排序同样是数值越大越靠前，新建默认 +10；“全部”分类只由前台生成，不写入数据库。
