...
- 完整 ESLint 规则继续覆盖 `admin/`，后台主脚本清理已无引用的旧地图坐标与表格辅助函数、未使用 catch 参数及冗余布尔转换；地图、表格、治理、API 和权限行为没有变化。
- `adminUpdates` 与后台日志同步本次维护，主脚本缓存 token 为 `20260908-site-review-r1`；后台样式和独立工具页面缓存不随本次等价清理变动。

> 管理后台专用上下文：只描述 `/admin/` 的运行与维护约定，不等同于主站项目文档。以下新约定优先于本文历史日期段落中已被替换的统计窗口或显示文案。

## 2026-09-08 统计、内容与治理工作台

- 前端统计拆到 `admin/analytics-workbench.js`，内容辅助能力拆到 `admin/content-workbench.mjs`，治理分页拆到 `admin/management-workbench.mjs`；服务端查询和待办投影位于 `functions/api/admin-query-service.mjs`、`admin-workbench-service.mjs`。三个受保护控制台共用 `admin/shared-console.css`，主页面保留原有模块与鉴权入口；本轮缓存标识为 `20260908-admin-review-r1`。
- 访问报表使用 `Asia/Shanghai` 自然日，前端提供 1／7／14／30 天且包含当前尚未结束的一天。API 返回 `range.start/end/todayStart/onlineStart`、`generatedAt`、真实 PV／UV／点击／消息序列和 SQL 去重覆盖数；日／小时零表示已采集事件为零，不证明完整真实流量为零。在线没有历史序列，不画装饰曲线。城市地图、页面／文章排行、访问来源与点击聚合均使用所选区间；排名仅声称已返回的范围。
- `collection.current` 只解释当前采样开关和比例，历史采样率没有完整留档，不能按当前比例倒推历史或声称可比增长；报表明确暂不展示同比／环比。D1 写入预算仍按 UTC 日计数和重置，不能随展示时区迁移。日期改变需等旧请求结束后读取新范围；失败保留旧快照并标明范围，自动刷新可暂停，手动刷新仍可用。CSV 包含区间／时区／采集说明和聚合数据，防公式注入，不扩展到明文身份或原始网络记录。
- 只读 `/api/admin/workbench-summary` 返回草稿、视频资料、待完成删除和本人 H3 失败任务计数，缺表／查询失败为 `null + unavailable`。已有事件汇成近期记录，并展示真实投递、互传清理与提醒状态；这些记录不是全部后台操作的完整审计。自动投递下一时间只按已有固定计划推算，通道启用和最近送达均不能证明外部执行器在线。
- 内容列表默认每页 50 条并使用服务端组合筛选、总数与分页信息；调整筛选或翻页仍经过未保存保护。文章预览使用安全 DOM Markdown 子集；本机暂存按账号 ID 摘要分区，最多 12 篇、每篇最多 5 份历史、单份 320 KiB、总计 2 MiB。它是浏览器 localStorage 明文副本，不是加密或跨设备历史；损坏／超额／浏览器禁用存储时明确失败并保留编辑器。恢复保留当前服务器 revision，先确认、再记录被替换内容，只更改编辑器，正式保存与发布继续验证三语。
- 聊天、禁言和账号列表支持分页；治理状态由服务端按实际生效／到期条件计算。聊天修改与账号资料／密码修改使用 `expectedUpdatedAt` 条件更新，消息／账号读取失败或版本冲突不得清空输入，也不能显示已保存；账号密码与资料操作保持独立。
- 主后台把关键指标、待办和趋势前置，重复排行折叠；同一组 token 统一独立控制台。响应式保留单一导航抽屉、列表／详情双态与主要操作命中区，减少动态偏好下立即执行关键切换。实现层的桌面／窄屏验证不能替代真实触屏或软键盘验收。
- Quick Transfer 管理改动触发独立版本 `1.0.13`，主站只同步版本和缓存。站内提醒只显示实际 Webhook 配置与接收结果，邮件和 Cloudflare 官方预算提醒仍须各自验证；H3 执行／传输默认关闭，真实家庭链路与 GPU 验证未完成前不宣称可执行。
...
- 旧书签不带参数时只允许用同源 referrer 推断；无法确认时回到公共工具区。该 UI 来源信息不参与管理员身份判断，也不能替代现有 `/admin/*` 会话与角色鉴权。
