...
- 访问报表和地图共用所选 `Asia/Shanghai` 自然日区间；前端提供 1／7／14／30 天，当前日／小时尚未结束。D1 预算保护继续按 UTC 日，不能为统一显示而改变计费重置口径。KPI 必须使用各自真实事件序列、保持时间顺序，在线只显示近 5 分钟已采集活跃快照；日／小时 UV 不可相加，覆盖总数用服务端去重统计，榜单长度只代表已返回结果。
- 报表的 `collection.current` 是此刻采样配置，不是历史采样证据；缺失历史基线时不要逆推完整流量或显示同比／环比。聊天消息统计来自当前仍保留的记录，删除／过期清理会影响历史数量。CSV 保留区间、时区、采集说明并防公式注入，不导出未授权身份或原始网络记录；刷新保留焦点／已展开内容，范围加载失败时不能把旧快照标成新范围。
- `/api/admin/workbench-summary` 只投影既有计数和事件，未建表／读取失败显示不可用；`deleting` 表示删除待完成，不能直接断言失败。投递开关、计划推算和最后送达都不能证明执行器心跳，已有投递／画板／互传／H3 记录也不是完整后台审计。
- 文章本机暂存是按账号 ID 摘要隔离命名空间的浏览器 localStorage 明文副本，不是加密云备份；限额、损坏和禁用存储必须真实反馈。恢复先确认并保留替换前的本机版本，只回填编辑器，不能用暂存的旧 revision 覆盖当前服务器基线或绕过三语正式保存校验。保存已成功但回读失败时保留当前输入与真实写入结果，不清空编辑器后标为已保存。
- 内容和治理分页必须使用同一服务端筛选与计数契约，区分“全量总数”和“当前页”；筛选、翻页与跳转继续走未保存保护。账号资料／密码和消息编辑／隐藏／删除都必须携带 `expectedUpdatedAt`，冲突保留输入。共享样式及拆分脚本变更同步全部使用页的缓存 query，后台私有更新仍只写后台更新系统。
...
- 当前互动地图使用与报表相同的所选日期区间，按国家、地区和城市精确分组且具备有效聚合经纬度；不得退回固定 14 天、国家中心随机兜底，也不得把不同城市合并成不可核对的装饰点。
