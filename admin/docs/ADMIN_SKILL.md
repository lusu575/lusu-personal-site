...
- 长表单的主保存操作需要持续可见。文章、视频、聊天和账号等可编辑表单必须维护 dirty 状态，并在切换模块、条目、新建、移动返回或刷新前提供保存、放弃、留下选择；保存失败不能清除 dirty 或输入内容。
...
- 没有筛选或详情动作的地图点位必须是非交互标记，不得伪装成可点击按钮；地图下方需要提供同一聚合数据的可读地区列表。
