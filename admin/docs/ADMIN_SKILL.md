...
- 本地生成适配层固定放在 `自动新闻/integrations/lusu-site/`：必须先运行 Horizon 原生抓取入口完成多源采集、网址规范化和跨来源去重，运行记录保存可反查的 `runId` 与候选文件；Horizon 不可用时停止，不能由 Codex 手工采集顶替。生产任务每天北京时间 07:00 开始，严格使用 `[前一日 07:00, 当日 07:00)` 的精确 24 小时窗口；Codex 负责复核、30 天跨日去重、重要性门槛、三语整稿、无外链正文和个人站投递格式。所有阶段必须在 08:00 前完成，失败或超时不得补发。数量不得写死，也不得为凑数生成空洞新闻。
