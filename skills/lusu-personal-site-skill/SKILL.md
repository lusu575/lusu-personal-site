...
- 四段开关的 scene、active node、semantic marker、single accent 和 frame 必须使用 `image2`／imagegen 生成的项目内位图及来源 manifest，禁止用 CSS、Canvas、SVG path 或代码几何临时绘制主体。176×44 椭圆在任一时刻只呈现当前时段的安静色场，不能恢复成四景并排拼轨；四个节点始终可见，未激活节点为各时段独有的 20×20 低对比浮雕语义标记，不能退回空白珍珠、通用圆点或透明圈；激活节点为 32×32 的半露晨日／正午全日／低位落日／月亮，固定 top 6px、left 6／50／94／138px。当前公开更新 ID 为 `seed-update-2026-08-10-wallpaper-switch-calm-redesign`，公共 token 为 `20260810-wallpaper-switch-calm-r1`；PNG 图像 token 独立保持 `20260809-wallpaper-time-switch-r4`。
- 每个主题只允许一个不同的稀疏 accent：morning 为四根分离短晨光、day 为一朵矮小云、dusk 为一条极薄短余晖、night 为七颗稀疏小星；day／night 基础 scene 必须保持无云／无星色场，禁止恢复 far／mid／accent 堆叠、行星、密集景物或错峰入场。day／night 未激活 marker 必须保持细线空心太阳／月牙，不得重新做成齿轮、实心珍珠或按钮。当前契约固定 17 个不同 Image2 最终内容源：4 scene、4 marker、4 node、4 accent、1 frame；来源 manifest 还要保留被弃用的生成调用、采用理由和只含 chroma helper、alpha bounds、透明定位、机械缩放的处理证据，不得用代码重绘或改色掩盖生成缺陷。若只校准既有 accent 在稳定态的可见位置，只允许透明画布整数像素平移，并必须锁定平移前后相同的 RGBA 裁切 SHA 与 alpha 像素数；不得新增生成调用、重采样或改色，且不因此升级未发布的公开／素材 token。16 个非 frame cell 按 manifest 顺序机械纵向打包为 `scene-atlas.png` 880×880、`marker-atlas.png` 144×576、`node-atlas.png` 192×768、`accent-atlas.png` 480×640，`frame.png` 独立，运行时固定 5 个唯一图片请求。不得重采样、混合、重复或跨语义复用 cell。快速重复触发的换档只允许可重定向的 transform／opacity transition，禁止 keyframes 与 stagger，并遵守 last-request-wins；single accent 必须 `transition-delay: 0ms`。目标图集只在 Home 需要时加载和预解码；键盘与 `data-motion="off"` 立即提交，reduced 只允许不超过 140ms 的纯 opacity，low performance／Save-Data 跳过 accent 但保留完整核心状态。手动选择的 pending request 必须持续拥有乐观档位和最终持久化提交，clock／focus／pageshow 只能复用；覆盖截止时间仍以点击瞬间计算，不能因解码跨过边界而延长。
