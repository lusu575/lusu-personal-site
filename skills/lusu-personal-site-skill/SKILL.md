...
- 桌面 Home 图标进入模块不得捕获整张 Home 页面，只在实时壁纸上对目标 `.xp-window` 做克制淡入和 3px 上移归位；任务栏返回 Home 只动画 `.desktop-icons`，不得让顶层 Home 快照遮住任务栏。模块间 `route` 只让新活动页面轻淡入并小幅方向滑动，旧窗口快照必须隐藏；移动 Dock route 使用短促方向滑动和一个共享选中底板。固定顶栏、桌面任务栏与移动 Dock 不参与页面快照，fallback 不得克隆带 ID 的业务 DOM。
...
- 音频的内容版本绑定迁移不得被 252 份耗时 Image2 生产长期阻塞。只有在 `audioSourceHash` 与 `sceneSourceHash` 证明 reading、voice、line／token／option、停顿和逐句 cue 等真实发音来源全部稳定时，才允许在同一 OS 发布锁下用 `jp-subtext:audio:rebind` 的 `prepare -> rebind -> check` 流程只更新 manifest／timeline 的内容绑定，并证明 MP3 字节集合未变；任何音频投影变化都必须回到 Aivis 重建。确需先迁音频时，内容构建必须显式传入 `--allow-legacy-illustrations` 并把状态标成 `transitional-audio-first` / `audio-first-legacy-illustrations`；默认构建、image2 检查和正式 release gate 仍严格拒绝 legacy illustrations。该过渡状态只能存在于候选分支，不得写成已合并 main、已上线或完整发布候选。
...
- “日本語の裏側”的关卡图只能由 `gpt-image-2`（image2）逐关生成，不允许 CSS、Canvas、SVG、程序化几何或本地 fallback 代替。每个 v4 prompt 必须逐项包含该关 setting、确定性设计身份卡、全部台词和题问，但不得包含选项、答案或解析；`image2/design-identities.json` 是人物连续性的唯一注册表，默认 identity 按关卡与 cast id 隔离，只有显式 alias 才可共享核心外观，variant 必须保持可辨，任务和 provenance 必须绑定注册表 schema、SHA-256、namespace 与确定性 design seed，禁止把通用 cast id 当成跨关同一人物。正式样式为高完成度黑白四格漫画，使用灰阶、网点、墨色与统一 2×2 边框，不能退化为裸线稿。远程／网络场景必须把不同地点放在完整主格中交替呈现，四个主格内部都禁止斜线分屏、内部边框、画中画或额外子格；审核发现视觉分格超过四个时必须拒绝。若某关反复生成错误拓扑、伪文字或泄题手势，只能把题面已有物理关系和禁止动作写入受测的稳定关卡 addendum 并重算该关 promptHash，禁止使用未入库的临时提示词。原始 PNG 只能通过项目内受测入口生成并保存在项目外目录；Codex 内置 `image_gen.imagegen` 与 OpenAI Images API 是互斥的真实证据通道，前者绑定 toolRunId/原始 SHA/状态为 `codex-approved` 的 Codex 六项视觉审核且明确不代表人类审核，后者从进程环境读取密钥并绑定真实 endpoint/request ID，两者均使用输出根写锁、逐图 provenance sidecar、原子替换、尺寸/SHA-256 验收、有界重试和断点复用，禁止记录密钥或静默覆盖无法验证来源的文件。原始 PNG、发布 WebP 与 manifest 必须锁定 model / quality / `sourceTextHash` 及其版本化投影 schema / promptHash / styleBibleHash / 设计身份注册表哈希 / SHA-256 / 尺寸 / dHash / 审查状态；`sourceTextHash` 只覆盖日英标题、场景、人物、说话者归属、台词和题问，排除插图、版本/revision、现有关卡 `contentHash`、选项、答案、解析和设计身份注册表，避免来源哈希被插图结果反向改写。所有图片响应式、懒加载且不得泄露答案。
- 内置 image2 调用读取日文 canonical prompt 时只能用 Node UTF-8，并在同一执行单元验证 `sha256(job.prompt) === job.promptHash` 后把同一个字符串直接传入工具；禁止 PowerShell 5.1 JSON 往返或人工复制。生成、审核、稳定源复制、review 与 raw 导入必须作为可恢复事务，失败不能覆盖最后一个完整证据 checkpoint；难关达到有界重试上限后记录失败并退回队列，不能阻塞其他资产。
- Codex 内置 image2 的视觉审核只能写为 `codex-approved`，保留每份记录原有的 Codex reviewer 标识，并绑定六项显式检查、toolRunId 与原始 PNG SHA-256；不得把 Codex 审核声称为人类审核，`human-approved` 必须被发布门禁拒绝。
