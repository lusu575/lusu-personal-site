...
- 本地 CLI 和 stdio MCP 使用账号持有者确认的设备码授权，令牌按 `content:read`、`transfer:read`、`transfer:write`、`transfer:delete`、`whiteboard:read`、`whiteboard:write`、`japanese-subtext:progress:read`、`japanese-subtext:progress:write` 最小化请求；默认不授予删除、画板或日语进度 scope。Quick Transfer 查看／下载属于 read，进房／发文字／上传属于 write，项目删除和分片上传中止属于 delete；画板 write 只隐含 read。设备码轮询遇到网络／中止或明确瞬态 HTTP 故障时，只能在设备码有效期内有界退避，且不得输出 token、代理或底层网络细节。Agent Bearer 令牌永远是普通机器角色，不能继承 admin 或访问管理接口；管理功能继续只接受 HttpOnly 浏览器会话。
