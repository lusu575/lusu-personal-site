...
- 注册的新用户和首个会话必须在同一 D1 batch 提交，提交成功后才能返回 Cookie。把成功登录记录和成功后的限流桶清理作为失败隔离的后台副作用；统计失败不能把成功认证改成失败，固定错误日志不得携带身份或凭据。
- 互传未发送内容仅属于当前房间内存，离房、关闭或退出登录时清空，不持久化明文草稿。隐藏标签只暂停被动轮询与不可见 UI 更新，保留用户提交上传、重试及队列；手动暂停和房间 generation 仍优先，旧请求不能影响新房间。不要把此行为描述为浏览器／系统永不挂起。
- 文章导航只拦截未处理的普通主键点击，保留 Ctrl／Cmd／Shift／Alt、新标签和原生链接语义。写入、公共投影与显示层的标签去重必须保留独立标签及首个原拼写；历史去重以原 tags 为 CAS 条件，不顺带改编辑时间。搜索仍是标题／摘要／标签的有界批次扫描，不能宣称正文全文索引或固定总成本。
- 移动回首页手势只从真实 44px Dock 指示条开始，在输入、软键盘、弹窗、缩放和失活状态下禁用；滑动成功后只抑制对应指针兼容点击，不吞键盘激活。把媒体断点例外放回所属组件，避免继续追加日期式覆盖；不要删除承载不同状态的同名选择器。
- 动效结束与取消必须服从最新请求：离页取消旧页面效果，快速开关从当前可见帧接续，旧主题计时器不能清除新状态。reduced／off、键盘和禁用控件按既有交互契约立即收尾，不能以动效延迟或重复业务提交。
- 画板密码帮助使用受控原生按钮，鼠标点击不能把 hover 说明锁定，移出关闭；键盘聚焦／Escape 与触屏切换／外部关闭均可用。展开内容留在正常文档流中，不能遮输入；继续复用既有 Image2 素材与画布协议。工具卡先讲用途、门槛和保留边界，技术接入细节使用可访问的次级展开入口。
- 第一方 JS／MJS 继续执行完整 ESLint recommended；按精确路径排除第三方引擎和生成物，本站桥接保持覆盖。只为实际安全校验等确有理由的单行规则写局部解释，不用目录级豁免掩盖新增错误，不因 Lint 清理擅自改变业务或许可证边界。
- `content-migrations.mjs` 是文章数据迁移入口；历史基线放 `article-seeds.mjs`，本期增量放 `article-release-seed.mjs`，成功后原子记录 `site_data_migrations` 与版本 marker。同 DB 合并并发，失败可重试，不能假称已消除未知旧库的首请求兼容迁移。周期清理只在全部过期数据追平后记录每日成功，失败／预算耗尽留下可续跑状态，并用 owner 租约避免并发覆盖。
- 发布必须核对完整 40 位预期 Git SHA：构建环境声明与 checkout 一致，生产 manifest 的 commit 匹配且 `dirty:false`，页面资源引用和实际字节哈希匹配，检查结束再次确认版本。使用 `EXPECTED_COMMIT_SHA` 运行生产 smoke；旧版本的 HTTP 200、资源 query 字符串或历史部署记录不能代替本次发布证据。

## 后台私有维护边界

- `/admin/` 的统计口径、浏览器暂存、并发写入与控制台样式由 `admin/docs/ADMIN_SKILL.md` 维护，不复制为公开主站功能。后台访问统计按 `Asia/Shanghai` 展示，D1 预算仍按 UTC；当前采样配置不能倒推历史，计划／最后投递也不能证明外部执行器在线。
- 仅因独立后台修改而触发 Quick Transfer 版本治理时，同步子项目版本与必要公开缓存；若公开功能和操作行为未变化，不要把后台私有细节写入公开 `site-updates`。实际发布与服务可用性继续以真实回读为准。
...
- 不要为了拦 `/wp-admin`、`.env` 等静态探测新增全站 Pages middleware，否则免费静态请求会被转成 Functions 调用。此类路径应在 Cloudflare `http_request_firewall_custom` 阶段拦截；创建前先读取现有规则、dry-run、避开本站真实 `/admin/`，创建后回读规则。只有只读权限或 dry-run 失败时明确记录未上线，不得伪报 WAF 已启用。
- 历史遥测默认不删除；后台时间窗和 180 天保留会自然淘汰。若站长要求清理，必须先给出精确候选数量与可恢复性，再取得针对删除的明确授权。

## MiniMax H3 工具区入口规则

- 公共 Tools 只展示站长专用的受保护启动卡，必须链接到 `/admin/minimax-h3.html` 并继续经过现有 `/admin/*` 管理员会话保护；不得代理或嵌入原版 ComfyUI UI。
- P0/P1 控制面可以实现严格 schema、D1 任务元数据、状态机、幂等、Runner register/heartbeat 与保守 admin 回读，但执行闸门默认关闭；不得用 0、假心跳、假队列或假输出冒充在线。P2 任务提交、家庭 Runner 完整轮询、Bridge、媒体传输和生产验收完成前不得宣称上线。
- ComfyUI 与 Bridge 固定为家庭回环地址 127.0.0.1:8188 和 127.0.0.1:8791；参考素材和成片不进入 Cloudflare 存储或 CDN。新图标等光栅素材使用 image2 生成并保留来源清单，MiniMax H3 图标必须明确表现 AI 视频生成主题，禁止用 CSS、SVG 或代码绘制替代。
...
- `workers/site-admin-mcp/` 是生产站长 OAuth remote MCP，canonical resource 固定为 `https://lusu575.com/mcp`；2026-08-09 由真实站长浏览器 OAuth Allow 完成验收的生产 Worker `lusu-site-admin-mcp` version ID 为 `fa295db6-302a-4a20-a2b1-ffe1ddafd75b`，Production D1 migration、OAuth metadata／DCR／401 challenge／Origin／pathname 线上 smoke，以及九个工具／四项公开 capability、原子发布、同载荷重放、管理回读、CAS 更新、三语公开回读、确认删除／三语 404 和 grant 撤销闭环均已通过，临时文章已删除。它必须继续使用 OAuth 2.1 authorization code、PKCE S256、精确 RFC 8707 resource、最小 `content:read`／`content:write`／`content:delete` scope、D1 grant／审计与每次管理员复核，绝不复用站点设备令牌或传递 `lusu_session`。每个新生产 Worker bundle 必须重新进行真实浏览器 OAuth 与同等完整闭环，不得用历史 bundle 的验收冒充当前版本成功。`workers/site-mcp/` 只保留四个公开读取工具的复用注册层与非 canonical 无 OAuth 目标；在线画板的受限追加／图片上传下载／导出和隔离游戏会话仍只代表本地 CLI／stdio MCP 能力，不得扩张为全站远程 MCP、远程画板写入、任意编辑删除或已打开浏览器的游戏接管。
- 仓库中的站长 MCP 0.3.0 源码候选新增独立 `videos:write` 和四个视频链接工具，但未部署时必须继续把上条九工具 bundle 写作现网真相。视频工具只登记现成 YouTube／Bilibili／b23 链接与平台封面／元数据，不下载或托管视频；默认草稿，首次公开要求 `confirmPublish: true`，写入用 operationId 收据、D1 原子 batch 和更新 CAS。新 bundle 必须先完成 Production D1 迁移，再重新取得站长真实 OAuth 与视频预览／登记／重放／回读／更新／公开回读／撤销证据。
...
- 四时段壁纸开关固定在桌面顶栏右侧，移动端只在 Home 顶栏下方右侧显示；轨道、选择器和其中的日月云视觉必须使用 `assets/images/wallpaper-switch/` 内登记来源的生成图像素材，不要用 CSS／Canvas／SVG 或 DOM 图形重新绘制。手动选择只覆盖到下一本地真实时段边界，随后必须自动清除并恢复随时间切换；覆盖优先使用 `localStorage`，不可用时由 `sessionStorage` 兜底，刷新要恢复，成功写入本地存储时要同步其他标签页。`?wallpaper=` 继续只作优先级更高的显式预览。
- 壁纸开关的细指针选择器保持 220ms strong ease-in-out，底图与动态云层必须作为同一场景执行 300ms generation-safe crossfade；快速连续选择必须以 generation 保证 last-request-wins，旧请求不得覆盖新状态。自动边界、跨标签和手动切换都要在提交前预解码目标场景；手动覆盖期限必须以点击瞬间的最近真实边界为准，不能因解码跨点延长。键盘与 motion-off 必须即时提交；`prefers-reduced-motion` 取消选择器位移并只保留约 140ms 的纯透明度场景过渡。该偏好是纯浏览器前端状态，不得为它新增 D1、MCP、能力注册、CLI 或远程 Worker 接口。
- 首页欢迎弹窗按本文件 2026-09-08 约定改为主动打开；保留每日问候和最近更新内容，默认访问及深链不自动弹出，`welcome=1` 只用于明确预览。
...
- 移动 Home 使用 App grid，真实 Dock 在 Home 与栏目 App 内持续悬浮；Dock 只保留 Home、Knowledge、Videos、Tools（内部 `resources` route）、Games、Chatroom 六个高频入口，About 仍从 Home 进入，Blog 则从整个移动壳下线。手机直达、History 恢复或桌面切换到移动壳时的 `#blog` 必须替换为 Knowledge；桌面端可选 Blog 逻辑保持不变。六项在 375px 以上居中，359px 可短距离横滑；进入排除路由时选中底板必须隐藏。Dock 仍可由 44px 横线收起/展开，栏目内只保留单一 Appbar。
- 移动 Home 的永久排除入口和可选入口 `hidden` 状态必须使用高于通用 tile `display` 规则的选择器保护，并以 computed style 回归，防止后加载的 `.desktop-icon { display: ... }` 把隐藏项重新显示成幽灵图标。
...
- 自动云同步间隔：10 分钟；手动点击“立即同步”必须绕过计时等待并立即尝试上传。
...
- 每日生成适配层固定放在 `自动新闻/integrations/lusu-site/`。任何一期开始前必须完整读取同目录 `ARTICLE_STYLE.md`，不得临时更换内容型标题、三段栏目、事实段或 AI 解读格式。三语文章标题必须分别使用固定栏目名前缀加各自正文第一条要闻标题，不能只写日期；日期继续由发布时间与 slug 表达。生产运行每天 `Asia/Shanghai` 07:00 触发，窗口左闭右开为 `[前一报告日已保存的采集启动时刻, 本次执行实际采集启动时刻)`；同日每次新执行都替换当前日期锚点并扩展到本次实际开始时间。随后运行 `npm.cmd run ai-news:horizon:fetch -- --date <报告日>`，由 Horizon 原生抓取器完成多源采集、网址规范化和跨来源去重，并把真实窗口、当前 `runId`、`daily_candidates.json`、紧凑 candidate index 与 coverage manifest v2 写进运行记录。`horizon.config.json` 当前虽指向本地 Ollama `qwen3.6:27b`，正式入口仍只调用抓取、来源重试与去重，不调用 Horizon 原生 AI 评分或富化；不得把“已有模型配置”误写成“模型已参与正式生产”。精确窗口内 candidate index 的全部候选都必须进入完整发现审阅，不能只审聚焦查询、指定来源或 priority。公开正文的标题后必须直接进入首个栏目，不显示摘要、采集窗口或筛选说明；每条新闻使用唯一三级标题，供文章目录逐条列出全部新闻标题，目录不得只列三个栏目名。自动任务的抓取、核验、三语生成、投递和受控公开允许在同一报告日的本次实际采集启动时刻至次日 00:00 完成；08:00 不再是硬截止。Horizon 不可用、无合格稿或校验失败仍停止且不发布，也不得以 Codex 手工浏览冒充自动采集。只有站长在交互任务中明确要求当天补发后，才可按 `MANUAL_RECOVERY.md` 使用日期与 canonical 稿件 SHA-256 双确认入口；v2 连续窗口、schemaVersion 4、Horizon／覆盖产物、三语、通道、auto-publish、幂等和公开回读门禁全部不变。
- 发现层不得按语言排除来源；可靠的中文、英文、日文、韩文及其他语言来源都可进入复核，并应使用重点实体的英中日韩常用别名帮助发现。required query／entity group 至少覆盖 Anthropic、OpenAI／GPT／Sam Altman／Codex 关键人物与产品运营变化、Thinking Machines／Inkling、LG AI Research／K-EXAONE 等开放模型与韩国模型实验室、Kimi／月之暗面、智谱／GLM、千问／Qwen／千问办公、MiniMax、DeepSeek、混元、美团 LongCat／CatPaw、微信 WeLM、字节跳动／豆包／Seed 等模型厂商，以及 Seedance 等视频、图像、语音多模态产品的发布、延期、API、权重与可用范围变化；芯片／光刻／存储、机器人、智能设备、数据中心能源／散热／网络和科技金融也在范围内。宽泛的大型 OR 查询只能补充召回，不能代替重点人物、产品或单一厂商的独立 required 查询。字节跳动固定拆分豆包中英产品动态、Seed 通用模型、SeedRealtime／Seed-ASR／Seed-TTS／全双工语音和 Seedance／Seedream／Dreamina 创意模型查询；综合 `bytedance-models-zh` 只作补充。Demis Hassabis 固定拆成 DeepMind 通用、Gemini、AlphaFold／Isomorphic、战略动态四条 required 查询，原人物宽查询只作补充；DeepSeek 固定拆成模型发布／开放权重与产品／API／价格额度两条 required 查询，原中文综合查询只作补充。
...
- 高流量 required 发现不能由一条跨事件宽查询承担。宽父查询保留为 supplemental，完整签收必须按实体、事件阶段、来源类别或动作意图拆成互补的 required／must-review 分片；自动驾驶至少分开热点实体直源与可靠媒体、其他运营商、监管变化和排除热点后的通用部署。配置变更前用目标精确窗口执行 max-plus-one 实探；任一分片触发第 100 条就继续缩窄，不能提高上限、把截断写成 empty／success、删除覆盖范围或临时跳过该语言。
- X 公开 profile 结构会在 schema.org meta 与 React relay 记录之间漂移。解析 relay 时不得执行页面脚本，只能用当前账号 DOM 中的规范 status ID 白名单关联 `full_text` 和 `created_at_ms`，再按精确半开窗口过滤；不得收录引用账号的内嵌 Tweet。页面已有 post article 但无法解析任何可验证记录必须报 failure，不能记作成功空结果；结构变化必须用真实页面回放和 fixture 双重验证。
- TechCrunch AI、VentureBeat AI、Ars Technica AI、雷峰网和 36氪只作可选补充。Reddit 与 Hacker News 只用于早期发现；每条返回候选仍须处置，但标题、评论与发帖时间不能单独支撑正式事实或时间资格，必须追溯到官方、可靠媒体或其他一手来源。站长已授权把 Tibo `@thsottiaux` 的 X 帖子纳入选题；移除会返回同名医疗噪声的 Bing RSS，改由 required 的 `codex-operations-en` 聚焦查询检查姓名、账号及 Codex／ChatGPT Work 运营变化，并以 required 的 `codex-usage-policy-en` 单独查用量限制、额度与重置。OpenCode 必须有独立 required 查询。为防止搜索索引延迟漏掉官方帖，正式抓取还要通过共享代理客户端读取 `public-x-profiles.json` 的指定公开主页，把帖子原始时间、规范 URL 与账号来源写入同一候选索引和全量审查；该补充只代表公开索引，不能声称是完整登录时间线或 X API。来自 `reddit-codex` 或这两条 Codex 聚焦查询的简短 reset 标题，即使省略产品名或额度词也必须进入 `usage-policy-change` 受保护审阅；明确属于密码、主题、编辑器、设置、配置或偏好的 reset 才排除。所有 X、媒体和社区线索最终仍须回到规范原帖、可靠媒体或其他一手来源核验。
- 从 report date 2026-08-10 起，每期公开日报必须至少包含 5 条独立、处于精确窗口、达到 7 分门槛并完成可靠核验的新闻。初选少于 5 条时必须启动低产量第二轮审阅和定向补查；二审后仍不足 5 条则本期 fail closed，不生成也不投递少于最低数量的文章。不得用重复事件、窗口外消息、未经核实的传闻或低价值内容凑数。跨日去重按 `eventKey + eventStage` 处理：同一事件同一阶段继续排除，正式发布、正式开源／开放权重等实质新阶段只有在记录前序故事和 material difference 后才可作为更新入选。正文固定为“今日要闻 / 主要新闻 / 传闻”，传闻只靠独立分区和条件语气区分，不逐条重复“未证实”。每条 AI 解读通常一至两句、明显短于正文，只挑关键影响、现实门槛、隐含限制或下一步观察点，不复述新闻，也不为了找问题而硬挑问题。来源 URL、评分与筛选理由只留在内部运行记录，对外正文不得出现网址、Markdown 链接、来源／参考资料章节、相关阅读跳转或内部评分。
- 正式日报使用 schema v4；`npm.cmd run ai-news:validate` 必须反查对应 Horizon 候选文件，验证 candidate index、coverage manifest v2、required query／entity group 签收、required 真实结果上限、candidate index 全部候选逐条处置、`sourceCandidateIds` 映射和低产量第二轮审阅，并检查三语标题均为固定前缀加各自第一条要闻标题、标题后没有公开导语、三段顺序、新闻三级标题唯一、单段事实正文、逐条一至两句且短于事实段的 AI 解读、传闻无重复核实标签。`runs/2026-07-28-coverage-revision.json` 的 manifest v1 仅是按固定 run、路径与 SHA-256 登记的 schema v4 历史兼容例外，不得复制、改写或作为新运行模板；除该精确身份外，正式 schema v4 必须使用 manifest v2 和覆盖全部候选的 `priorityReview`。schema v3 只允许已登记的一次性历史样稿兼容，不得用于新的正式运行；历史 CI 可显式传入仓库内紧凑 provenance fixture，但该入口必须同时校验 one-shot 开关、schemaVersion 3 与登记窗口，schema v4 必须拒绝。不得让 CI 依赖 Git 忽略的完整本地 Horizon 运行目录，也不得删除或绕过格式、来源和覆盖证明。本地试投只允许通过 `npm.cmd run ai-news:deliver:local` 使用进程内临时令牌并走正式机器入口；无论成功失败都要停止预览、暂停通道和清除令牌。2026-07-27 样稿是生产链路测试输入，不能绕过相同的令牌、幂等、冲突、验证和同报告日午夜截止规则。
- 自动新闻正式 Node 外联必须复用 `自动新闻/integrations/lusu-site/network-fetch.mjs`，由精确锁定的 Undici 读取 `HTTP_PROXY`／`HTTPS_PROXY`／`NO_PROXY`，兼容本机 Clash Fake-IP 和无代理直连环境；不得记录代理值、代理凭证或投递 Token，也不得依赖修改 Clash 配置才能发布。生产 POST 每次执行最多一次；接口明确返回 published 后，三语公开核验只允许对网络错误和 408／425／429／500／502／503／504 做每语言最多 3 次、受同一报告日午夜截止约束的只读 GET，正文不匹配、非瞬时状态或耗尽后仍 fail closed，绝不重发 POST。
...
- Knowledge 使用 `/api/articles?paginated=1` 的服务端游标分页，默认每页 12 条、最多 48 条；分类计数覆盖全部已发布文章，分页游标绑定语言、分类、排除分类与搜索。保留已加载页和阅读返回位置，分类／搜索切换中止旧请求且拒绝旧响应回写；不要恢复依赖旧接口 500 条上限的完整归档假设。搜索按 NFKC 多词 AND 扫描摘要批次，保留三语标签匹配与精确计数，不把它描述为持久全文索引。
...
- 如果网站更新记录通过 seed 维护，同步 `functions/api/article-release-seed.mjs` 的本期增量、`content-migrations.mjs` 版本、`cloudflare/schema.sql`、`js/data/content.mjs` 完整 fallback 与 `js/data/home-content.mjs` 最近五条无正文投影；历史基线只在真实历史修复时改动，避免线上 D1、migration、Home 首屏与离线兜底显示不一致。
...
- 视频 Agent／远程 MCP 同样只做“链接登记”：共享元数据服务必须有界抓取白名单平台 HTML／JSON／oEmbed，服务端派生 `platform`、`external_id`、`original_url`、`embed_url`；不得下载视频二进制，也不得让调用方伪造平台字段或把任意 URL 变成 iframe。
- `/api/agent/videos*` 与远程视频工具使用独立、非默认、管理员专属的 `videos:write`，每次复核当前管理员；不能直接转发 cookie 绑定的 `/api/admin/videos*`。登记默认 `draft`，从非公开状态进入 `published` 必须显式 `confirmPublish: true`。
- 视频登记和更新必须以 `operationId + canonical caller-intent payload SHA-256` 幂等；远程元数据不进入哈希。视频行、分类关系、Agent 审计和 `agent_video_receipts` 在同一 D1 batch，更新还必须使用最新 `expectedUpdatedAt` CAS；Agent 封面 URL 只允许平台官方图片域名，浏览器后台的受限本地 data-image 封面不自动扩张到 MCP。
- `videos(platform, external_id)` 目前只有普通检索索引，因此所有写入通道都必须在最终写 SQL 中再次执行 source 不存在条件，不能只依赖写入前查重；后台更新必须先完成带 CAS 与 source guard 的视频 UPDATE，再让分类删除／插入依赖新的 `updated_at`，保证失败时不留下分类部分修改。
