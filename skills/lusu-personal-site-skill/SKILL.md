...
- YouTube 单链接补全先请求官方 oEmbed；失败或缺标题时，只能用已校验的 11 位 videoId 构造固定官方 watch URL 兜底，不能跟随调用方提供的任意页面或把任意 iframe 当元数据源。provider JSON／HTML 必须流式计数并设独立小上限，超时要覆盖响应正文而不只是 headers；当前上限为 256 KiB／2 MiB。解析结果仍需走标题／简介／作者长度与控制字符规范化、发布时间校验及官方缩略图 host 白名单。任何 provider 联网都必须晚于 caller-intent 收据回放。
