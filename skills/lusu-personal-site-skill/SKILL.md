...
- 四段开关的场景、天体、节点标记、边框和氛围层必须使用 `image2`／imagegen 生成的项目内位图及来源 manifest，禁止用 CSS、Canvas、SVG path 或代码几何临时绘制主体。176×44 椭圆在任一时刻只呈现当前时段的完整场景，不能恢复成四景并排拼轨；四个节点始终可见，未激活节点为各时段独有的 18×12 实体标记，激活节点为 32×32 的半露晨日／正午全日／低位落日／月亮，固定 top 6px、left 6／50／94／138px。禁止通用透明圈。
- 每个当前主题使用不同的 scene 与 far／mid／accent 内容，禁止复制同一视觉区域或 atlas cell 冒充多个语义层；morning 用晨光射线与云，day 用白云，dusk 用落日云带，night 用丰富星群与行星。允许将不同的 Image2 内容 cell 按 manifest 顺序机械纵向打包：25 个内容源保留逐项尺寸／SHA，24 个内容打包为 7 张 delivery atlas，`frame.png` 独立，运行时固定 8 个唯一图片请求；不得重采样、混合、重复或跨语义复用 cell。night 三层按 3／5／6px 上升并以 0／35／70ms 错峰。快速重复触发的换档只允许可重定向的 transform／opacity transition，禁止 keyframes，并遵守 last-request-wins。目标图集只在 Home 需要时加载并预解码，非 Home 路由不得产生这些请求。键盘触发与 `data-motion="off"` 立即提交；reduced 只允许不超过 140ms 的纯 opacity，low performance 直接落到静态目标。手动选择的 pending request 必须持续拥有乐观档位和最终持久化提交，clock／focus／pageshow 只能复用；覆盖截止时间仍以点击瞬间计算，不能因解码跨过边界而延长。
