...
- `workers/site-mcp/` 当前只是未部署的公开只读远程 MCP Worker；在没有真实发布和线上验证前不得宣称存在正式远程入口。任何远程账号或写能力必须先实现标准 OAuth、最小 scope、撤销与审计，不得把本地设备令牌直接暴露给公网 MCP。在线画板的受限追加／图片上传下载／导出和隔离 2048 会话已可从本地 CLI／stdio MCP 使用，但不代表远程写入、任意画板编辑删除或已打开浏览器的游戏接管；其余能力仍须等 `availableTransports` 与安全回归同时落地才可宣告。
- GPTWork 每日 AI 新闻使用独立 `workers/daily-ai-news-mcp/`，绝不能把写工具加入公开只读 `workers/site-mcp/`。该 Worker 必须由 Cloudflare Access Managed OAuth 与 owner-only policy 保护，并在源站验证 JWT signature、issuer、audience、必需时间／身份 claims、精确 owner email、专用 Host 和无浏览器 Origin；只能只读核对现有 `daily-ai-news` channel 的 `enabled + auto_publish`，并只向既有 `articles` 写一行、`article_translations` 写恰好 zh／en／ja 三行。不得绑定 KV、R2、DO、Queue 或新 D1，也不得写 delivery event、channel usage、候选、草稿、checkpoint 或新闻缓存；固定标签仍为 `每日AI新闻 + AI`。同日同稿重放幂等、异稿冲突，且成功前必须从 `lusu575.com` 三语公开 API 精确回读 slug、category、status、语言、标题、摘要和正文；回读必须携带真实跨站 Worker 来源，让公开 API 的 read-source gate 跳过 article-view event、visitor profile 与 view-count 写入。结构合格或客户端自报 validator 成功不构成无人值守安全边界；服务端能验证完整临时运行包，或能验证与日期和最终三语内容 hash 绑定的短时可信签名回执之前，只允许站长逐次确认的交互试投，`remote-mcp` 不得进入 `availableTransports`，GPTWork 定时任务必须成稿后失败关闭。平台安全／请求日志按平台保留设置处理，不得把“不写应用缓存”夸大成绝对零平台存储。
