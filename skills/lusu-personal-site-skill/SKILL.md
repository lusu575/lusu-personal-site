...
- 0.4.0 候选只收窄既有 `video_publish` 的必填输入：工具总数保持 23，scope 保持 `content:write`，成功语义保持直接 `status=published`。AI 只提交唯一 `operationId` 与 YouTube／Bilibili／b23.tv `originalUrl` 时，服务端有界补全省略的标题、简介、作者、发布时间和平台官方封面；既有直接公开边界不变。精确 Worker version 与真实 OAuth 最小载荷闭环必须部署后回填，候选文档不得提前宣称上线。
...
- 当前四个时段的静态舞台仍是永久兜底，视频壁纸与 CSS 动态云则必须互斥：`assets/images/wallpaper-dynamic/<time>/base-clean.png` 始终可见；只要当前环境具备视频播放资格，就不得创建或预热独立透明云层，由第一版 MiniMax H3 整帧视频自身承载云层运动。视频未就绪或失败时显示静态舞台，不能再叠加第二套动态云；真正不具备视频资格的旧路径才可按原策略使用 CSS 云层。
...
- 静态壁纸是永久兜底；视频只有在 canplay／play 成功后才能短淡入，加载或自动播放失败不得留空白。手机、low performance、Save-Data、`prefers-reduced-motion`、`data-motion="reduced"` 与 `off` 全部必须零视频请求。离开 Home 或页面隐藏只暂停并保留当前节点、`src` 与解码器，返回后复用同一节点；只有真正失去播放资格或主题／分辨率资源变化时才释放。当前修复记录 ID／slug 为 `seed-update-2026-08-12-wallpaper-game-display-fix`／`2026-08-12-wallpaper-game-display-fix`，公开 API／文章 seed token 为 `20260812-wallpaper-game-display-r1`；8 月 11 日第一版 H3 与 8 月 10 日局部合成记录继续作为历史保留。
...
- 共享游戏壳的 frame card 必须占满扣除安全边距后的可用宽度，不得用固定 1280px `max-width`，否则浏览器缩放会把游戏压成中央小窗。宽度不超过 860px 或高度不超过 720px 时，存档／云端、许可证与 AI 工具默认收起到一个至少 44px 的三语按钮后，让 iframe 获得剩余空间；不得用全局 `zoom`、裁切或缩小触控目标绕过真实响应式布局。
...
- `video_publish` 的 `title`、`description`、`thumbnailUrl`、`authorName`、`publishedAt` 可省略并由固定平台 provider 有界补全；显式字段优先，包括允许的空字符串或 `null`。标题最终必须非空：调用方未提供且 provider 无法取得时返回 `VIDEO_METADATA_TITLE_UNAVAILABLE`，视频、分类关系、收据与审计必须零写入；显式标题存在而其他元数据抓取失败时，可使用安全默认值并写入受限 `metadata_error`。远程抓取结果不得进入 caller-intent hash。
- YouTube 单链接补全并行请求官方 oEmbed 与由已校验 11 位 videoId 构造的固定官方 watch URL：规范化后的 oEmbed 标题／作者／缩略图优先，页面补简介／发布时间并可兜底标题；页面的 canonical／`og:url` 必须回指同一 videoId，不能跟随调用方提供的任意页面、接受通用错误页或把任意 iframe 当元数据源。provider JSON／HTML 必须流式计数并设独立小上限，超时要覆盖响应正文而不只是 headers；当前上限为 256 KiB／2 MiB，畸形、超限或中断读取必须主动取消响应流且不得掩盖原始错误。解析结果仍需走标题／简介／作者长度与控制字符规范化、发布时间校验及官方缩略图 host 白名单。任何 provider 联网都必须晚于 caller-intent 收据回放。
