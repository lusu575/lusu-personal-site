...
- `site-guides` 是固定的“网站使用指南 / Website Guides / サイト利用ガイド”知识库分区；即使暂时没有文章也保留入口，固定排在 `daily-ai-news`、`tool-radar` 之后、普通分类和 `site-updates` 之前。这里用于口语化、简单的站内功能攻略；把多个工具写在同一篇文章时必须明确它们仍是独立功能，不能暗示房间、密码或状态互通。涉及真实界面时优先使用匿名临时状态下取得的电脑端与手机端实拍图，截图不得包含密码、访客消息或个人信息，文章图登记原始尺寸并使用内容哈希 query。
