...
- Pages 全局 mutation gate 必须在精确同源检查之后，同时识别且只识别 `POST /api/whiteboard/assets` 与 `POST /api/whiteboard/agent/assets` 两个 raster 入口，MIME 仅限 PNG／JPEG／WebP；不得用前缀匹配、通配路径或宽泛 `image/*`。安全的无 Origin CLI 请求可继续进入 Agent 鉴权，但不能绕过 Bearer、write+assets、tokenId 绑定房间令牌或大小／容器校验；回归必须分别锁定 Agent 缺令牌为 401、跨源为 403、相邻路径与 SVG／其他 MIME 为 415。
