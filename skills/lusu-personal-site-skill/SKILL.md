...
- 浏览器游戏 WebSocket 的待机／暂停保活只允许精确应用层文本 `ping`／`pong`：当前 Worker 必须先以 Cloudflare Hibernation `setWebSocketAutoResponse("ping", "pong")` 配置边缘自动应答，Pages 页面每 8 秒发送 `ping` 并忽略精确 `pong`。保活不得唤醒 Durable Object、读取 provider、生成 observation／action、改变 revision 或写入 DO 存储；精确 `pong` 只作传输活性确认，不能把保活伪装为观察、控制或状态持久化。Pages-only 改动的验收绑定精确 Worker version 与最终 Pages commit；任何后续新 Worker bundle 仍须逐款跑完配对、动作、暂停、玩家恢复、确认关闭与撤销，静态／单元测试不能替代真实闭环。
