...
- 四段开关的 scene、active node、semantic marker、accent、frame 和 roller 必须使用 `image2`／imagegen 生成的项目内位图及来源 manifest，禁止用 CSS、Canvas、SVG path 或代码几何临时绘制主体。176×44 控件使用完整暖象牙陶瓷椭圆壳，内外轮廓必须同心对齐，中心开口 alpha bbox 锁定为 `[36,29,842,190]`，运行时可视区使用 `clip-path: inset(5px 6px round 17px)`。未选中节点必须是四种高辨识、光滑拟物的时段图标，不得回退到圆点、珍珠、透明圈或像素化轮廓。选中轮为 36×36，top 4px，四档 x 为 4／48／92／136px；32×32 天体以 2px inset 放在轮内并始终正向。独立外圈每档滚转 140deg，必须和 thumb 共用 `transform var(--motion-window) var(--motion-ease-in-out)` 的可中断 transition；不得用 keyframes、`transition: all`、ease-in 或任何布局属性伪造滚动。当前公开更新 ID 为 `seed-update-2026-08-10-wallpaper-switch-ceramic-roll`，公共 token 为 `20260810-wallpaper-switch-ceramic-roll-r1`，PNG 图像 token 为 `20260810-wallpaper-time-switch-r5`。
- 每个主题只允许一层不同的 accent：morning 是晨光与一朵小云向上展开，day 是两朵云水平横移，dusk 是低位余晖横向展开，night 是八颗稀疏星从下向上升起；四张基础 scene 不嵌入太阳、月亮、云、星或地景，禁止恢复 far／mid／accent 堆叠、行星、密集景物或 stagger。当前契约固定 18 个不同 Image2 最终内容源：4 scene、4 marker、4 node、4 accent、frame 和 roller；manifest 必须保留完整真实 prompt、call ID、生成源路径与 SHA-256、弃用调用及原因、官方 chroma helper、机械 alpha trim／等比缩放／透明定位过程和 QA，不得臆造或用代码改色。除 frame 外的 17 个内容 cell 按 manifest 顺序机械纵向打包为 `scene-atlas.png` 880×880、`marker-atlas.png` 144×576、`node-atlas.png` 192×960、`accent-atlas.png` 480×640，其中 roller 固定为 node atlas 第五格；只有 `frame.png` 独立，运行时固定 5 个唯一图片 URL，禁止单独请求 `roller.png`，也不得通过放宽 Transfer trace 预算掩盖额外请求。任何 cell 都不得重采样、混合、重复或跨语义复用。图集只在 Home 需要时加载和预解码；键盘与 `data-motion="off"` 立即提交，reduced 移除 thumb／roller 位置运动且其他层只允许不超过 140ms 的纯 opacity，low performance／Save-Data 跳过 accent 但保留完整核心状态。pending request、last-request-wins 与点击瞬间的下一边界语义必须保持不变。
