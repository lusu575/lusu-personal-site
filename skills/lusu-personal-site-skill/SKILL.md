...
- `workers/site-mcp/` 是未部署的公开只读 Worker 目标；站长写能力必须放在独立 `workers/site-admin-mcp/`，使用 OAuth 2.1 authorization code、PKCE S256、精确 RFC 8707 resource、最小 scope、D1 grant／审计与每次管理员复核，绝不复用站点设备令牌或传递 `lusu_session`。仓库实现、dry-run 或本地全流程通过都不等于线上可用；只有真实 KV／Secret／D1 迁移、Worker 发布和正式域名 OAuth 写入闭环完成后，才可把 `remote-mcp` 加入对应 `availableTransports` 并对外公布地址。在线画板的受限追加／图片上传下载／导出和隔离 2048 会话已可从本地 CLI／stdio MCP 使用，但不代表远程写入、任意画板编辑删除或已打开浏览器的游戏接管。
