...
- 桌面 Home 图标进入 App 不得使用整张 Home 快照，只在实时壁纸上淡入目标窗口；任务栏返回 Home 只动画 `.desktop-icons`，不得让 Home 快照盖住任务栏。模块间 route 只让新活动页面轻淡入并小幅方向滑动，旧窗口快照隐藏；手机 Dock 使用短促方向滑动和一个共享选中底板。固定顶栏、桌面任务栏与移动 Dock 不参与页面快照，fallback 不克隆业务 DOM。
- 移动 Home 使用 App grid；真实毛玻璃 Dock 在 Home 与栏目 App 内持续悬浮，只保留 Home、知识库、视频、资源、游戏、聊天室六个高频入口，杂谈与关于仍从 Home 进入。375px 以上六项居中，359px 可短距离横滑；排除路由不显示伪选中底板。Dock 仍通过 44px 横线收起/展开，主要触控目标至少 44x44 CSS px。
...
- 全站指针视差关闭；慢速壁纸氛围只能使用与输入无关的 `transform` / `opacity`，并在页面隐藏、减少动态或关闭动效时回到静态。大面积页面/窗口/弹层禁止动画 `filter`、`box-shadow`、`border-radius` 或使用 3D 翻页，只允许短促 `transform + opacity`；`reduced` / `off` 立即完成状态变化。
- 新增或修改 `mobile-ios-shell.css`、`motion-system.css`、`mobile-shell.js`、`ui-motion.js` 或其他公开 CSS/JS/强视觉资产时，必须同步 `index.html` 对应 query。
- 移动竖版壁纸和位图 UI 素材必须由 image2 产出并复制到项目资产目录；网页无法读取真实设备状态时不得显示模拟信号、Wi-Fi 或电量图标。本地 QA 截图与临时生成文件不得提交。
