...
- 工具雷达规则固定在 `自动新闻/integrations/lusu-site/tool-radar/`：正式任务已获授权，固定于北京时间每周二 22:00 启动；每期目标 6–10 个、少于 3 个不投递。逐项用官网或可靠一手来源核对功能、费用、登录、中文、本地／AI 辅助部署、用法、案例与场景，并生成 zh／en／ja 完整文章。`tool_radar_catalog` 以 `tool_key` 和规范官网 URL 阻止精确重复并与文章原子落库；疑似改名、换域名或被收购的产品还须人工核对历史名称和别名，未排除重复前不得投递。同类不同产品允许后续介绍。首期 `trial + not-delivered` 只作历史审阅依据，不能原地改为 production。图片只引用 `assets/images/articles/` 的安全项目内资源，不热链；以每个工具一张有语义的说明图为正常目标、最多两张，站点资源先由 GitHub `main` 部署到 Cloudflare Pages，生产投递再核对线上可读取性与远程 SHA-256。专用通道、凭证和 auto-publish 保持独立，只有两项显式闸门都开启时才自动公开。
