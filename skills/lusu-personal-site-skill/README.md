...
自 2026-09-04 起，定时任务仍在上海时间 07:00 触发，但新闻窗口改为“前一报告日已保存的采集启动时刻→本次执行实际采集启动时刻”的连续半开区间，不再锁死整点 24 小时。每次同日报告重跑都替换本日期锚点并向后推进右边界，左边界仍取上一报告日锚点；所有 Horizon 产物、schema-v4 稿件、正式校验和人工恢复都核对 v2 策略、同一对时间与当前 run id，缺少上一日锚点时停止。
...
公共工具区的 Online ComfyUI · MiniMax H3 只允许作为站长专用的受保护启动卡，链接到 `/admin/minimax-h3.html`，不得公开或嵌入原版 ComfyUI UI。P0/P1 可实现严格协议、D1 元数据、状态机与真实心跳回读，但执行闸门默认关闭，必须显示未接入/未验证状态；ComfyUI 与 Bridge 只计划使用家庭端 127.0.0.1:8188 与 127.0.0.1:8791，媒体字节不进入站点存储。新图标等光栅素材统一使用 image2 生成并保留来源清单，MiniMax H3 图标须明确表现 AI 视频生成主题，禁止代码绘制。
...
- 站长 MCP 0.3.0 源码候选已实现独立 `videos:write` 下的视频链接预览、登记、管理读取和 CAS 更新，但本轮未部署，现网仍是上条九工具 bundle。该能力只把现成 YouTube／Bilibili／b23 链接及平台元数据／官方封面登记到本站，不下载或托管视频；默认草稿，公开需 `confirmPublish: true`，并以 operationId 收据、原子 D1 batch 和 CAS 防重。新 bundle 必须重新迁移和真实 OAuth 验收后才可宣告上线。
- 视频来源去重不能只做“先查后写”：`videos(platform, external_id)` 仍是普通索引，Agent 与浏览器后台的最终 INSERT／UPDATE 都要在写 SQL 内保留 source guard；更新应先提交带 CAS 的主行，再让分类关系依赖新的 revision，避免竞争失败后留下部分分类修改。已公开卡片替换来源同样必须重新确认公开。
...
- Blog 已从移动壳下线，Mobile Home 不显示也不聚焦其入口，手机直达／History／桌面转移动壳的 `#blog` 统一替换为 Knowledge，桌面端可选 Blog 逻辑不变。移动 CSS 必须以足够特异性守住明确排除和原生 `hidden`，并用 computed style 回归防止后加载的 `display` 规则制造幽灵图标。
...
- 四时段壁纸开关在桌面位于顶栏右侧、移动端只在 Home 顶栏下方右侧；轨道、选择器及其日月云视觉使用 `assets/images/wallpaper-switch/` 内登记来源的生成图像素材，不用 CSS／Canvas／SVG／DOM 重绘。手动选择只保留到下一本地真实时段边界，再恢复自动切换；覆盖优先存入 `localStorage`，不可用时以 `sessionStorage` 兜底，刷新可恢复，成功写入本地存储时通过 `storage` 事件同步其他标签页，`?wallpaper=` 仍是更高优先级的显式预览。
- 细指针选择器使用 220ms strong ease-in-out，底图与动态云层作为同一场景使用 300ms generation-safe crossfade；快速选择必须按 generation 实现 last-request-wins。自动边界、跨标签和手动切换先预解码目标场景，手动期限按点击瞬间的最近真实边界固定。键盘与 motion-off 即时提交；减少动态取消选择器位移并只保留约 140ms 的纯透明度场景过渡。该开关保持纯前端，不接入 D1、MCP、能力注册、CLI 或远程 Worker；当前公开缓存 token 为 `20260809-wallpaper-time-switch-r1`。
...
- TechCrunch AI、VentureBeat AI、Ars Technica AI、雷峰网和 36氪是可选补充。Reddit 与 Hacker News 只用于早期发现；它们返回的每个候选都要处置，但标题、评论和发帖时间不能单独支撑正式事实或时间资格，必须追溯到官方、可靠媒体或其他一手来源。站长已授权把 Tibo `@thsottiaux` 的 X 帖子纳入选题；已移除会返回同名医疗噪声的 Bing RSS，改由 required 的 `codex-operations-en` 聚焦查询检查姓名、账号及 Codex／ChatGPT Work 运营变化，并以 required 的 `codex-usage-policy-en` 单独查用量限制、额度与重置；OpenCode 另有独立 required 查询。正式抓取还通过共享代理客户端读取 `public-x-profiles.json` 的指定官方公开主页，把帖子原始时间、规范 URL 与账号来源写进同一候选索引和全量审查，以补足搜索索引延迟；该补充只代表公开索引，不是完整登录时间线或 X API。来自 `reddit-codex` 或这两条 Codex 聚焦查询的简短 reset 标题，即使省略产品名或额度词也必须进入 `usage-policy-change` 受保护审阅；明确属于密码、主题、编辑器、设置、配置或偏好的 reset 才排除。所有 X、媒体和社区线索最终仍须回到规范原帖、可靠媒体或其他一手来源核验。
...
- 正式自动新闻的 Node 网络请求统一经过 `network-fetch.mjs`：直接依赖的 Undici 会读取 `HTTP_PROXY`／`HTTPS_PROXY`／`NO_PROXY`，兼容 Clash Fake-IP 和普通直连环境，代码与日志不得暴露代理值、代理凭证或投递 Token。生产 POST 每次执行只能发送一次；服务端确认 published 后，三语公开核验可对网络错误和明确瞬时 HTTP 状态执行每语言最多 3 次、受同一报告日午夜截止约束的只读 GET，正文不匹配、非瞬时错误或重试耗尽仍停止，不能因此重发 POST。
...
- 本地试投使用进程内临时令牌走正式机器接口，结束后必须关闭预览、暂停通道并清空令牌。2026-07-27 样稿用于生产链路测试，必须遵守与正式运行相同的来源证明、令牌、限流、幂等、冲突、失败关闭和同报告日午夜截止规则；不因自动公开授权而保存模型或第三方密钥。
...
- 已确认新闻使用 6 分线并要求可靠直达证据；从 report date 2026-08-24 起传闻使用 5 分线且不设最低数量，所有达到相应门槛的独立内容仍须保留。传闻可由有归属的当事人公开预告、一篇有明确归属的可靠直达报道或两家独立可靠媒体直达报道支撑，并必须保留条件语气、具体 `whyUnverified` 与 `rumorEvidenceBasis`，不得用聚合页、匿名截图、重复事件或纯猜测凑数。required discovery 单独覆盖 Tibo／Codex 预告、xAI Grok 产品、Qwen 开放权重、LTX 视频模型与英文 DeepSeek 发布；selected／merged priority 处置必须写自身 `eventKey + eventStage`，标题中的竞品提及不能造成跨事件合并。
- 2026-08-14 起的正式链路在 Horizon 后固定运行 `ai-news:semantic-review`：校验 candidate index 实际字节 SHA-256，对全候选做内容语义处置，再对信号、RSS 与受保护类别按事件阶段做证据复核；中断后从同一索引 checkpoint 续跑。自动任务不再附加 `--automatic-deadline`，默认使用四路受控并发和 32／16 紧凑批次，并允许在同一报告日内完成；08:00 不再触发中止。只有完整台账与已核实三语事实包可交给 `ai-news:assemble` 生成 schema-v4 记录。不得用标题正则、固定分数、候选 ID／数组位置或循环模板生成审稿结论；语义审阅端点或已配置模型不可用时必须 fail closed。
- 事件层评分使用明确的全球／重大市场 reach、变化幅度、即时实用性和证据强度锚点，并用既往已发摘要与本轮先前入选事件持续去重。人工终审若纠正模型结论，必须在三语事实包的 `eventOverrides` 留下精确事件身份：拒绝写四项低于 6 分的具体理由，同事件同阶段别名精确合并到目标事件并同步合并候选来源，不能静默删除。
