...
- 四段开关的 scene、active node、semantic marker、single accent 与 frame 只使用 `image2`／imagegen 生成的项目内位图及来源 manifest，不用 CSS／Canvas／SVG path／代码几何绘制主体。176×44 椭圆任一时刻只显示当前安静色场，禁止四景拼轨、空白珍珠、通用圆点和透明圈；四节点常驻，未激活标记为各时段独有的 20×20 低对比浮雕语义标记，其中 day／night 保持细线空心太阳／月牙，不能变成齿轮或实心按钮；激活天体为 32×32 的半露晨日／正午全日／低位落日／月亮，top 6px、left 6／50／94／138px。每个主题只允许一层不同的稀疏 accent：morning 四根短晨光、day 一朵矮小云、dusk 一条极薄短余晖、night 七颗稀疏小星；day／night 基础 scene 必须无云／无星，禁止 far／mid／accent 堆叠、行星、密集景物或 stagger。当前 17 个不同最终内容源逐项锁定尺寸与 SHA；manifest 同时保留弃用生成调用及理由、官方 chroma helper 和机械 alpha／缩放证据，禁止代码改色。既有 accent 若只校准稳定态位置，只允许透明画布整数像素平移，并锁定前后相同的 RGBA 裁切 SHA 与 alpha 像素数；不得新增生成调用、重采样或改色，也不升级同一未发布版本的 token。16 个非 frame cell 机械打包为 scene 880×880、marker 144×576、node 192×768、accent 480×640 四张 atlas，另加独立 `frame.png`，运行时固定 5 个唯一图片请求；不得重采样、混合、重复或跨语义复用 cell。快速换档只用可重定向的 transform／opacity transition，禁止 keyframes，accent 延迟固定 0ms；图集仅在 Home 需要时加载和预解码。键盘与 motion-off 即时，reduced 只保留不超过 140ms 的纯 opacity，low／Save-Data 跳过 accent 但保留完整核心状态；pending request、last-request-wins 与点击瞬间的下一边界语义保持不变。当前发布记录为 `seed-update-2026-08-10-wallpaper-switch-calm-redesign`，公共 token 为 `20260810-wallpaper-switch-calm-r1`，PNG 图像 token 保持 `20260809-wallpaper-time-switch-r4`。
