...
- 视频 MCP 第一阶段只读取／管理 YouTube／Bilibili／b23.tv 外链记录；精确候选 bundle 的闭环成功也不能替代最终 availability promotion bundle 的重验，`videos_list`／`video_get` 和六项站长工具在最终 bundle 验收前都不能加入远程可用面。写入使用管理员复核、审计、`operationId` 持久收据、CAS 和确认删除。远程 MCP 不读取本机路径、Base64、原始字节或客户端文件；`video_upload_sessions` 预留 schema 不表示 R2 文件上传已经配置，真实上传须另建私有二进制数据面并完成分片、配额、扫描、提交、中止和清理验收。
