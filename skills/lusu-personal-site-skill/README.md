...
- 浏览器游戏连接保活固定为当前 Worker 先以 Cloudflare Hibernation `setWebSocketAutoResponse("ping", "pong")` 提供精确边缘自动应答，Pages 页面每 8 秒发送精确文本 `ping` 并忽略精确 `pong`；不得唤醒 DO、读取 provider、生成 observation／action、修改 revision 或写存储，也不得把保活描述成观察。Pages-only 改动须绑定精确 Worker version 与最终 Pages commit 逐款重跑；后续任何新 Worker bundle 也须重新完成完整闭环。
