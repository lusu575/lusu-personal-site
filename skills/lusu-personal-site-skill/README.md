...
- “日语的言外之意 / Behind the Japanese / 日本語の裏側”位于 `tools/japanese-subtext/`，固定采用版本化分批 JSON 和不可随意变更的关卡 ID；每次公开应用更新增加 `appVersion`，只有题库结构、内容哈希边界或存档兼容变化时才增加 `contentVersion`，改关另增 `revision`，完整流程见 `tools/japanese-subtext/MAINTENANCE.md`。
...
- 音频内容绑定迁移与耗时 Image2 生产必须解耦：仅当稳定 audio／scene source 投影证明真实发音输入和 cue 未变时，才可在同一 OS 锁下执行 `jp-subtext:audio:rebind` 的 `prepare -> rebind -> check` 并证明 MP3 字节未变；否则回到 Aivis 重建。音频优先中间态必须显式使用 `--allow-legacy-illustrations`，写明 `transitional-audio-first`，且默认构建、image2 与 release gate 继续严格；候选不得误写成已合并 main、已上线或完整发布候选。
