...
- 四段开关的完整场景、激活天体、未激活标记、边框和氛围层只使用 `image2`／imagegen 生成的项目内位图与来源 manifest，不用 CSS／Canvas／SVG path／代码几何绘制主体。176×44 椭圆任一时刻只显示当前主题，禁止四景拼轨与通用透明圈；四节点常驻，未激活标记为 18×12，激活天体为 32×32 的半露晨日／正午全日／低位落日／月亮，top 6px、left 6／50／94／138px。每个主题使用不同的 scene 及 far／mid／accent 内容，禁止复制同一视觉区域或 cell 冒充多层；允许按 manifest 将 24 个不同 cell 机械纵向打包为 7 张 delivery atlas，另加独立 `frame.png`，运行时固定 8 个唯一图片请求。25 个内容源仍逐项锁定尺寸与 SHA。night 三层按 3／5／6px 上升、0／35／70ms 错峰。快速换档只用可重定向的 transform／opacity transition，禁止 keyframes；图集仅在 Home 需要时加载和预解码。键盘与 motion-off 即时，reduced 只保留不超过 140ms 的纯 opacity，low performance 直接显示静态目标；pending request、last-request-wins 与点击瞬间的下一边界语义保持不变。
