...
- `site-guides` 固定显示为“网站使用指南 / Website Guides / サイト利用ガイド”，即使暂时没有文章也保留入口，固定排在 `daily-ai-news`、`tool-radar` 后、普通分类和 `site-updates` 前。专区文章保持简单口语化；合写多个工具时要明确功能不互通。真实界面图片同时覆盖电脑端和手机端，不得留下密码、访客消息或个人信息，并登记尺寸与内容哈希 query。
