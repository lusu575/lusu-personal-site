...
- 每日 AI 新闻适配层位于 `自动新闻/integrations/lusu-site/`：每期先完整读取 `ARTICLE_STYLE.md`，再用带 `--start` / `--end` 的 `npm.cmd run ai-news:horizon:fetch` 真实调用 Horizon 做多源采集、网址规范化和跨来源去重。生产运行每天北京时间 07:00 开始，窗口严格是 `[前一日 07:00, 当日 07:00)`；全部抓取、复核、三语生成、验证、投递和受控公开必须在 08:00 前完成。运行记录保留可反查窗口、`runId` 与候选文件；Horizon 不可用、没有合格新闻、验证失败或超时就停止且不发布。`horizon.config.json` 虽已指向本地 Ollama `qwen3.6:27b`，当前正式入口只执行抓取、来源重试与去重，不调用 Horizon 原生 AI 评分或富化，不得误报模型已经参与生产。条数不写死；三语标题固定为栏目名前缀加各自第一条要闻标题，不得只写日期，日期由发布时间与 slug 表达。标题后直接进入“今日要闻”，不显示摘要、采集窗口或筛选说明；正文固定“今日要闻 / 主要新闻 / 传闻”，每条新闻使用唯一三级标题，网站目录据此逐条列出全部新闻标题，不只列栏目名。一段事实正文和更短的一至两句 AI 解读由校验器强制执行，传闻不逐条重复“未证实”，外部三语文章不得含网址、参考资料、来源列表或内部评分。
- 正式日报使用 schema v4 和 coverage manifest v2：Horizon 产物必须含紧凑 candidate index，编辑运行必须签收 required query／entity group，并逐条处置来自聚焦查询和指定 RSS／社区发现源的全部 must-review 候选；初选少于 5 条强制第二轮审阅与定向补查，但 5 条不是最低配额，严禁凑数。发现不限制来源语言，宽泛查询只作补充，重点人物／产品运营变化和各家中国模型厂商使用独立 required 查询；高流量 required 产品动态查询还必须按厂商和语言拆分，正式入口默认回看 24 小时并只按实际启动时刻扩到完整覆盖精确窗口，不能固定多抓 48 小时让窗口外旧消息占满单条上限。Google News 最多保留 99 条并请求第 100 条作为结果上限探针，实际返回第 100 条才判截断。重大模型／产品、能力／可用性、用量规则、开发工具和显著价格额度变化达到 7 分后不能仅因“产品型”而排除，入选项用 `sourceCandidateIds` 反查索引。跨日去重继续使用 `eventKey + eventStage`。`runs/2026-07-28-coverage-revision.json` 的 manifest v1 只是一份按固定 run、路径与 SHA-256 登记的 schema v4 历史兼容例外，不得复制或用作新运行模板；其他正式 schema v4 运行必须使用 manifest v2 和完整 `priorityReview`。schema v3 只保留给已登记的历史 one-shot 样稿。历史 CI 只可显式使用仓库内紧凑 provenance fixture，并同时校验 one-shot、schemaVersion 3 和登记窗口；schema v4 必须拒绝，不能依赖 Git 忽略的完整本地 Horizon 运行目录。
- TechCrunch AI、VentureBeat AI、Ars Technica AI、雷峰网、36氪与无需账号的 Tibo 公开检索源是可选补充：单个补充源失败不阻断，已抓到的候选必须审阅，成稿事实仍回到可靠或一手来源核验。当前没有直接接入 Tibo 的 X；以后启用 X 直连必须先取得用户授权。
...
- 工具雷达规则固定在 `自动新闻/integrations/lusu-site/tool-radar/`：正式任务已获授权，固定于北京时间每周二 22:00 启动；每期目标 6–10 个、少于 3 个不投递。逐项用官网或可靠一手来源核对功能、费用、登录、中文、本地／AI 辅助部署、用法、案例与场景，并生成 zh／en／ja 完整文章。`tool_radar_catalog` 以 `tool_key` 和规范官网 URL 阻止精确重复并与文章原子落库；疑似改名、换域名或被收购的产品还须人工核对历史名称和别名，未排除重复前不得投递。同类不同产品允许后续介绍。首期 `trial + not-delivered` 只作历史审阅依据，不能原地改为 production。图片只引用 `assets/images/articles/` 的安全项目内资源，不热链；以每个工具一张有语义的说明图为正常目标、最多两张，站点资源先由 GitHub `main` 部署到 Cloudflare Pages。生产投递按登记顺序逐张预检，对瞬时网络／瞬时 HTTP 故障最多三次有界尝试，最终仍须严格核对 200、MIME、大小与远程 SHA-256。专用通道、凭证和 auto-publish 保持独立，只有两项显式闸门都开启时才自动公开。
...
- 工具雷达以每个工具一张有明确语义的图片为正常目标、最多两张；每期必须先按 `VISUAL_METHOD.md` 写视觉任务卡，再取图、裁切与验收。单图连同图注要独立说清工具、动作／步骤和结果；双图只组成输入／输出、操作／结果、前后或连续步骤，并保持同组、先后明确和信息互补。关键标题、输入、动作、输出及结果不得遮挡或截断；三秒测试、正文宽度可读性、版权、隐私、稳定性和本地资产检查全部通过后才能进入三语正文。失败就重截、改本站原创说明图或三语一起删图，不能用空截图、泛化 Hero 或随意裁切填版面；临时官网审稿截图不等于已获正式转载权，正式投递还必须验证 Pages 线上文件 SHA-256 与运行记录完全一致。
