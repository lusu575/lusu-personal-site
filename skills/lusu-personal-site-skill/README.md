...
- 浏览器游戏连接保活的精确 `ping`／`pong` Pages 心跳只能覆盖浏览器持续发帧的场景；生产 Chrome 标签后台约 5 分钟强节流已证明，不能据短时通过宣称暂停闭环完成。paused 补救只允许已鉴权、owner／grant 绑定的 controller observe 验证 browser socket 后发送原始 `pong`，失败即断线；成功只更新 `lastControllerAt` 并返回缓存快照，不更新 `lastBrowserAt`、不读 provider、不生成 observation／action、不改 revision、不扩协议，也不能替玩家恢复。只有持续轮询获得保活（生产 helper 1.5–10 秒，客户端契约不超过 20 秒）；它不是服务端定时器，停止轮询不保证永久保活。候选必须标注未部署／未验收，并绑定每个精确 Worker version 与 Pages commit 逐款重跑完整闭环。
