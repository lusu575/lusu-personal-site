...
- AI 能力层以 `lib/capabilities/registry.mjs` 为唯一声明源：`transport` 是目标面，只有 `availableTransports` 是已实现且可对外承诺的真实面；复合授权由 `requiredScopes` 全满足与 `anyOfScopes` 至少一个表达。CLI、本地 MCP 和远程 MCP 共享适配服务，不复制业务规则；本地已提供受限画板追加／图片／导出、集成式隔离 2048／人生重开，以及独立 GPL 进程中的隔离 Hextris。当前生产 `849d8328-87db-4ac8-819a-ce725fc06349` 承载四款游戏 browser bridge 与视频外链工具候选；视频闭环只对历史 `377d494b-8f90-40ad-998f-863d209e1978` 通过，当前 bundle 必须重验，四游戏闭环也仍待重跑。视频条目的 `availableTransports` 尚未包含 `remote-mcp`，游戏条目的 `availableTransports` 仍为空。工具存在不等于可以扩大公开承诺。
- 0.4.0 候选不新增工具或 scope：现有 23 项工具、`content:write` 和 `video_publish` 名称不变，发布仍直接写 `status=published`。AI 只给唯一 `operationId` 与 YouTube／Bilibili／b23.tv 链接即可；标题、简介、作者、发布时间和官方封面省略时由服务端有界补全，显式覆盖优先。精确 Worker version 与真实 OAuth 闭环仍待部署后记录，不能把候选写成已上线。
...
- 隔离本地游戏会话和浏览器接管是两条数据面。2048／人生重开与专用 Hextris 会话继续使用 CAS、action ID 去重、状态／TTL 上限和破坏性确认；2048、Hextris、A Dark Room、人生重开的浏览器候选只允许 current-revision opaque `actionId`，拒绝 selector／script／raw key／coordinate／URL／DOM／原始存档。生产可用前还必须完成玩家显式一次性配对、单 pending command、可见暂停／收回／关闭、`games:play` OAuth、管理员复核、DO 中继和真实浏览器闭环。
- 浏览器游戏连接保活的精确 `ping`／`pong` Pages 心跳只能覆盖浏览器持续发帧的场景；生产 Chrome 标签后台约 5 分钟强节流已证明，不能据短时通过宣称暂停闭环完成。paused 补救只允许已鉴权、owner／grant 绑定的 controller observe 验证 browser socket 后发送原始 `pong`，失败即断线；成功只更新 `lastControllerAt` 并返回缓存快照，不更新 `lastBrowserAt`、不读 provider、不生成 observation／action、不改 revision、不扩协议，也不能替玩家恢复。只有持续轮询获得保活（生产 helper 1.5–10 秒，客户端契约不超过 20 秒）；它不是服务端定时器，停止轮询不保证永久保活。部署与验收必须分开标注：当前 `849d...` 已部署该逻辑但未完成四游戏验收；每个精确 Worker version 仍须绑定 Pages commit 逐款重跑完整闭环。
- 浏览器游戏命令只有在最终 outcome 审计持久化后才算完成；调用尝试与 `success`／`pending`／`error` 分开记录，审计失败返回 `MCP_OAUTH_AUDIT_FAILED`，不能误报成功。相同 `clientActionId` 的重试必须命中 DO 收据并回放既有结果，不得重复执行浏览器动作。
...
- Windows 许可证完整性测试只允许先把 CRLF 规范为 LF，再校验固定正文 SHA-256 和副本全文；不得以平台兼容为由跳过 hash、只匹配标题或做更宽松的正文归一化。
...
- 生产站长 OAuth remote MCP 位于 `workers/site-admin-mcp/` 和 `https://lusu575.com/mcp`；当前 version ID 为 `849d8328-87db-4ac8-819a-ce725fc06349`，内部版本 `0.3.1`，承接 100% 流量。该精确 bundle 的 metadata、401 与非 allowlist pathname 404 已通过，但业务生命周期尚未重验。历史 `fa295db6-302a-4a20-a2b1-ffe1ddafd75b` 的九工具知识库验收不能跨 bundle 复用，视频生命周期也只对历史 `377d494b-8f90-40ad-998f-863d209e1978` 通过；四游戏真实闭环未完成，视频条目的 `availableTransports` 尚未包含 `remote-mcp`，游戏条目的 `availableTransports` 仍为空。它继续使用 OAuth 2.1 + PKCE、精确 resource、最小 scope、D1 grant／审计和管理员实时复核，不得复用设备令牌或会话 Cookie；每个新生产 Worker bundle 必须重新完成对应闭环。`workers/site-mcp/` 继续只作公开读取工具的复用注册层与非 canonical 无 OAuth 目标。详细边界见 `docs/agent-capabilities/README.md` 和 `docs/agent-capabilities/REMOTE_MCP_CONNECT.md`。
- Kittens Game 因 WET PAWS LICENSE 固定为 `NO_AGENT`；没有上游明确许可／法律确认时不得增加 bridge、配对、动作或存档 Agent，也不能因其余四款游戏已接入而绕过。
- 视频 MCP 第一阶段只读取／管理 YouTube／Bilibili／b23.tv 外链记录；精确候选 bundle 的闭环成功也不能替代最终 availability promotion bundle 的重验，`videos_list`／`video_get` 和六项站长工具在最终 bundle 验收前都不能加入远程可用面。写入使用管理员复核、审计、`operationId` 持久收据、CAS 和确认删除；`video_publish` 必须在 provider 联网前先检查 caller-intent 收据，同意图重试直接回放。远程 MCP 不读取本机路径、Base64、原始字节或客户端文件；`video_upload_sessions` 预留 schema 不表示 R2 文件上传已经配置，真实上传须另建私有二进制数据面并完成分片、配额、扫描、提交、中止和清理验收。
...
- RFC 7009 access-token-only 撤销不能误撤 D1 grant；refresh-token 撤销须在 provider 删除前精确核对固定版本 grant 记录中的 current／previous token hash，并先写 grant 级确定性 D1 `pending` intent；标准撤销响应成功后仍显式确认幂等的整 grant 删除完成，以抵御并发 refresh 轮换导致的成功空操作，随后再以同一 batch 原子写 revoked 与唯一成功审计。跨 provider／D1 的失败恢复由强一致 D1 intent 驱动，不依赖最终一致 KV 删除后反查；不得保存原始 token，D1 失败返回稳定错误并允许同请求恢复，不能虚报成功或泄露 client secret／原始表单。
...
- 五游戏共享壳固定为一个 `100dvh` 网格，外层不滚动，iframe 使用剩余空间；frame card 占满扣除安全边距后的宽度，不设固定 1280px `max-width`。宽度不超过 860px 或高度不超过 720px 时，存档／云端、许可证和 AI 工具默认收起到至少 44px 的三语按钮后；不得用全局 `zoom`、裁切或缩小热区。359×500、390×844、844×390 必须测量外层滚动、iframe 可达性以及返回／存档控件 44px 热区。
...
- 四段开关的 scene、active node、semantic marker、accent、frame 与 roller 只使用 `image2`／imagegen 生成的项目内位图及来源 manifest，不用 CSS／Canvas／SVG path／代码几何绘制主体。176×44 控件保留四个 44px 目标；r6 暖象牙陶瓷外壳实测开口 half-open alpha bbox 为 `[18,20,861,201]`，可视区裁切为 `inset(4px 4px round 18px)`。四种未选节点要高辨识、光滑拟物，禁止圆点、珍珠、透明圈、齿轮和像素化轮廓；清晨必须使用低位暖橙半日、晨雾／地平线与薰衣草到杏桃色场，和高位完整太阳、鲜明蓝天的白天在 20／32／176×44 实际尺寸明显区分。wrapper 仍为 36×36、top 4px、四档 x 为 4／48／92／136px；32×32 天体以 2px inset 保持正向，独立 roller 可见外径约 33.375px、开孔 30px、壁厚约 1.6875px，每档滚转 151.072deg，累计 0／151.072／302.144／453.216deg。thumb 与 roller 共用 `transform var(--motion-window) var(--motion-ease-in-out)` 的可中断 transition，禁止 keyframes、布局属性动画、`transition: all` 和 ease-in。morning 晨光+小云上展、day 双云横移、dusk 余晖横展、night 八星上升，每个主题只有一层 accent 且延迟 0ms；禁止恢复四景拼轨、多层氛围、密集景物或 stagger。当前正式内容仍为 18 个不同 selected 源；被替换的 5 个 r5 源须连同完整 prompt／call／SHA 保留为 superseded 历史。17 个非 frame cell 仍机械打包为 scene 880×880、marker 144×576、node 192×960、accent 480×640 四张 atlas，roller 是 node atlas 第五格，只有 `frame.png` 独立；运行时固定 5 个唯一 URL，不单独请求 `roller.png`，不得放宽 Transfer trace 预算。这 5 个 URL 在开关实际可见的每个桌面路由预解码，开关内部动效不得以非 Home 为理由关闭；移动 App 紧凑栏隐藏时跳过额外 accent，整幅壁纸 crossfade／动态云仍为 Home-only。键盘／motion-off 即时，reduced 移除位置运动，low／Save-Data 跳过 accent；自动／手动边界、pending request 与 last-request-wins 不变。该开关自身的精修记录为 `seed-update-2026-08-10-wallpaper-switch-slim-dawn`，公开 API／文章 seed／main token 为 `20260810-wallpaper-switch-route-motion-r1`，素材 manifest 发布 token 保持 `20260810-wallpaper-switch-slim-dawn-r1`，PNG 图像 token 为 `20260810-wallpaper-time-switch-r6`。
- 首页保留 `wallpaper-root` / `wallpaper-stage` 舞台坐标结构和动画 layer DOM/class；morning / day / dusk / night 四时段继续用无云底图 + 独立 CSS 云层作为永久兜底，桌面 Home 在严格渐进增强门槛下叠加当前主题的第一版 MiniMax H3 整帧视频。
...
- 首页的 CSS 壁纸图层只使用 `transform` / `opacity`。整屏视频只对四时段壁纸开放严格渐进增强例外：仅桌面 Home + normal performance + Save-Data 关闭 + full motion 可请求，并且只按物理显示尺寸加载当前主题的 1080p 或 2160p muted／loop／playsinline MP4，其他三段不预载。视频播放资格成立时不得创建或预热 CSS 动态云，视频失败时显示常驻静态底图；手机、low、Save-Data、`prefers-reduced-motion`、reduced／off 全部零视频请求。页面隐藏或离开 Home 只暂停并保留视频节点／`src`／解码器，返回后续播；只有真正降级或主题／分辨率资源变化时释放。正式内容使用用户确认的第一版 H3 素材帧，严格按 `0..62 + 61..1` 组成约 5.17 秒、24fps 的整帧往返序列，不引入小女孩或电视 cameo；该序列先经双向光流补到 48fps、248 帧，再用 `RealESRGAN_x4plus_anime_6B` 对全部帧逐帧 AI 超分，输出 1080p／2160p。不得误写为使用原始 124 帧全段，不得恢复第二版静态底一次超分 + 局部 mask／gain 差分方案，也不得用重复帧冒充补帧。当前修复记录为 `seed-update-2026-08-12-wallpaper-game-display-fix`，公开 API／文章 seed token 为 `20260812-wallpaper-game-display-r1`；此前 H3 记录只作为历史保留。
