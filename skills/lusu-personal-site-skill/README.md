...
- 聚焦通道的新候选会以中英日韩变化词标记模型／产品、能力／可用性、开发者工具、价格／额度、芯片／存储／机器人／智能设备／自动驾驶／数据中心基础设施、重大科技金融和 AI 监管／安全 `editorialSignals`，并保留用量规则信号。信号强制正确受保护类别与深审，不强制刊发；`below-importance-threshold` 必须对应 `substantiveChange: true`，`no-material-change` 必须对应 false。对 50 条以上候选全部归为 `other`、或 90% 以上拒稿共用一个完全相同的编辑类别与四项评分模板的新运行，校验器直接关闭投递。少于 5 条的二审必须严格晚于初审，并在 `reconsideredCandidateIds` 中至少列全部带信号候选、RSS 候选与受保护类别的 5／6 分拒稿，同时可以增加其他索引候选；5 条仍只是复审触发器，不是配额。
