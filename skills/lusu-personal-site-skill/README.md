...
- Pages 主 mutation gate 对图片上传必须在同源检查之后精确覆盖浏览器 `/api/whiteboard/assets` 与 Agent `/api/whiteboard/agent/assets` 两个 POST，只接受 PNG／JPEG／WebP。不得用路径前缀或 `image/*` 放宽；CLI 无 Origin 只表示继续进入 Agent Bearer、write+assets、tokenId 房间令牌与图片校验。测试固定覆盖缺令牌 401、跨源 403、相邻路径和非 raster 415。
