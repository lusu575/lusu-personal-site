...
- 四段开关的 scene、active node、semantic marker、accent、frame 与 roller 只使用 `image2`／imagegen 生成的项目内位图及来源 manifest，不用 CSS／Canvas／SVG path／代码几何绘制主体。176×44 控件的暖象牙陶瓷外壳必须完整，内外椭圆对齐，中心开口 alpha bbox 为 `[36,29,842,190]`，可视区裁切为 `inset(5px 6px round 17px)`。四种未选节点要高辨识、光滑拟物，禁止圆点、珍珠、透明圈、齿轮和像素化轮廓。选中轮为 36×36，top 4px，四档 x 为 4／48／92／136px；其 32×32 天体以 2px inset 保持正向，独立陶瓷外圈每档滚转 140deg。thumb 与 roller 共用 `transform var(--motion-window) var(--motion-ease-in-out)` 的可中断 transition，禁止 keyframes、布局属性动画、`transition: all` 和 ease-in。morning 晨光+小云上展、day 双云横移、dusk 余晖横展、night 八星上升，每个主题只有一层 accent 且延迟 0ms；禁止恢复四景拼轨、多层氛围、密集景物或 stagger。当前 18 个不同最终内容源逐项锁定：4 scene、4 marker、4 node、4 accent、frame 和 roller；manifest 必须保留完整真实 prompt、call ID、生成源路径／SHA-256、弃用原因、官方 chroma helper、机械处理和 QA，禁止臆造或代码改色。17 个非 frame cell 机械打包为 scene 880×880、marker 144×576、node 192×960、accent 480×640 四张 atlas，roller 是 node atlas 第五格，只有 `frame.png` 独立；运行时固定 5 个唯一 URL，禁止单独请求 `roller.png`，不得放宽 Transfer trace 预算。任何 cell 都不得重采样、混合、重复或跨语义复用。图集仅在 Home 需要时加载与预解码；键盘／motion-off 即时，reduced 移除位置运动且其他层只保留不超过 140ms 的 opacity，low／Save-Data 跳过 accent；pending request、last-request-wins 与点击瞬间下一边界语义不变。当前发布记录为 `seed-update-2026-08-10-wallpaper-switch-ceramic-roll`，公共 token 为 `20260810-wallpaper-switch-ceramic-roll-r1`，PNG 图像 token 为 `20260810-wallpaper-time-switch-r5`。
