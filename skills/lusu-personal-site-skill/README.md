...
- 首页的 CSS 壁纸图层只使用 `transform` / `opacity`。整屏视频只对四时段壁纸开放严格渐进增强例外：仅桌面 Home + normal performance + Save-Data 关闭 + full motion 可请求，并且只按物理显示尺寸加载当前主题的 1080p 或 2160p muted／loop／playsinline MP4，其他三段不预载。手机、low、Save-Data、`prefers-reduced-motion`、reduced／off 全部零视频请求；页面隐藏暂停，离开 Home 或降级时释放视频，静态壁纸永久兜底。正式内容使用用户确认的第一版 H3 素材帧，严格按 `0..62 + 61..1` 组成约 5.17 秒、24fps 的整帧往返序列，不引入小女孩或电视 cameo；该序列先经双向光流补到 48fps、248 帧，再用 `RealESRGAN_x4plus_anime_6B` 对全部帧逐帧 AI 超分，输出 1080p／2160p。不得误写为使用原始 124 帧全段，不得恢复第二版静态底一次超分 + 局部 mask／gain 差分方案，也不得用重复帧冒充补帧。当前公开记录为 `seed-update-2026-08-11-h3-first-version-video-sr-48fps`，公开 API／文章 seed token 为 `20260811-h3-first-version-video-sr-48fps-r1`；8 月 10 日记录只作为历史保留。
