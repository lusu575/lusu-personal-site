...
export async function handleAgentArticlesApi(context, parts) {
  ...
  try {
    ...
    await requireCurrentArticleAdmin(env, principal.user.id);

    const route = parts.slice(2);
    if (request.method === "GET" && route.length === 0) {
      return await listAgentArticles(request, env);
    }
    if (request.method === "GET" && route.length === 1) {
      return await getAgentArticle(env, route[0]);
    }

    if (request.method === "POST" && route.length === 1 && route[0] === "publish") {
      await ensureAgentArticleReceiptSchema(env);
      return await publishAgentArticle(request, env, principal);
    }
    if (request.method === "PUT" && route.length === 1) {
      await ensureAgentArticleReceiptSchema(env);
      return await updateAgentArticle(request, env, principal, route[0]);
    }
    if (request.method === "DELETE" && route.length === 1) {
      await ensureAgentArticleReceiptSchema(env);
      return await deleteAgentArticle(request, env, principal, route[0]);
    }
    throw new AgentArticlesError(
...
async function requireCurrentArticleAdmin(env, userId) {
...
async function listAgentArticles(request, env) {
  const query = parseAgentArticleListQuery(request);
...
async function getAgentArticle(env, articleIdValue) {
...
async function publishAgentArticle(request, env, principal) {
  const payload = normalizePublishPayload(await readStrictJson(request));
  ...
  return agentArticlesJson(responsePayload, 201);
}

async function updateAgentArticle(request, env, principal, articleIdValue) {
  const articleId = normalizeArticleId(articleIdValue);
  const payload = normalizeUpdatePayload(await readStrictJson(request));
  const payloadHash = await hashCanonicalPayload({
    action: "update",
    articleId,
  ...
  return agentArticlesJson(responsePayload);
}

async function deleteAgentArticle(request, env, principal, articleIdValue) {
  const articleId = normalizeArticleId(articleIdValue);
  const payload = normalizeDeletePayload(await readStrictJson(request));
  const payloadHash = await hashCanonicalPayload({
    action: "delete",
    articleId,
...
function parseAgentArticleListQuery(request) {
  ...
  const category = rawCategory === null ? "" : normalizeCategory(rawCategory);
...
function normalizeSlug(value) {
  if (typeof value !== "string") {
    throw new AgentArticlesError("Article slug is invalid.", 400, "ARTICLE_SLUG_INVALID");
  }
...
function normalizeTags(value) {
  if (!Array.isArray(value) || value.length > MAX_TAGS) {
    throw new AgentArticlesError("Article tags are invalid.", 400, "ARTICLE_TAGS_INVALID");
  }
...
function normalizeBoolean(value, message, code) {
  if (typeof value !== "boolean") {
    throw new AgentArticlesError(message, 400, code);
  }
  return value;
}
...
function normalizeBoundedString(value, maxChars, options = {}) {
  if (typeof value !== "string") {
    throw new AgentArticlesError(options.message, 400, options.code);
  }
...
function assertStrictObject(value, allowedFields, code = "ARTICLE_PAYLOAD_INVALID") {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new AgentArticlesError("Article payload is invalid.", 400, code);
  }
  const allowed = new Set(allowedFields);
  if (Object.keys(value).some((key) => !allowed.has(key))) {
    throw new AgentArticlesError("Article payload contains unknown fields.", 400, code);
  }
}
...
async function readStrictJson(request) {
  ...
  if (!request.body) {
    throw new AgentArticlesError("Article request JSON is invalid.", 400, "ARTICLE_JSON_INVALID");
  }
...
function agentArticleAuditStatement(env, principal, event) {
  ...
  `).bind(
    ...
    event.action,
    event.articleId,
    JSON.stringify(normalizedScopes(principal.scopes)),
    event.result,
    event.createdAt
  );
}
...
function replayAgentArticleReceipt(receipt, action, payloadHash) {
  ...
  return agentArticlesJson({ ...payload, duplicate: true });
}
...
function articleNotFoundError() {
  return new AgentArticlesError("Article was not found.", 404, "ARTICLE_NOT_FOUND");
}
...
function agentArticlesErrorResponse(error) {
  if (error instanceof AgentArticlesError) {
    return agentArticlesJson({
      error: error.message,
      code: error.code,
      ...(error.details || {})
...
class AgentArticlesError extends Error {
  constructor(message, status, code, details = null) {
    super(message);
    this.name = "AgentArticlesError";
    this.status = status;
    this.code = code;
