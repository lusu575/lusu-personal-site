...
3. `functions/api/article-release-seed.mjs`、`functions/api/content-migrations.mjs`，以及 `functions/api/article-seeds.mjs` 的历史基线
...
Query provenance and direct-source provenance are separate authorities: derive `queryIds` and `mustReviewQueryIds` from original Google News topic items or an explicit public-X profile `queryId` reconciled against the current authoritative query catalog. Public X/RSS source IDs and source review lanes flow through direct-source provenance. The public-X fetcher must not invent required/must-review query policy; required Tibo links must retain query provenance, while a same-URL or profile link must never promote a supplemental query to must-review.
