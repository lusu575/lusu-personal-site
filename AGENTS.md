...
Query provenance and direct-source provenance are separate authorities: derive `queryIds` and `mustReviewQueryIds` only from original Google News topic items, while public X/RSS source IDs and review lanes flow only through direct-source provenance. A same-URL cross-source merge must never promote a supplemental query to must-review.
