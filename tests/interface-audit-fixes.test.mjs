...
test("the website-guide release leads the five-item trilingual projection while older Knowledge updates remain archived", async () => {
  const updateId = "seed-update-2026-08-06-site-guides-password-rooms";
  ...
  assert.equal(content.updates[1].article_id, trafficUpdateId);
  assert.equal(homeContent.updates[1].article_id, trafficUpdateId);
  assert.equal(content.updates[2].article_id, calmWhiteboardUpdateId);
  assert.equal(homeContent.updates[2].article_id, calmWhiteboardUpdateId);
  assert.equal(content.updates[3].article_id, reliableWhiteboardUpdateId);
  assert.equal(homeContent.updates[3].article_id, reliableWhiteboardUpdateId);
  assert.equal(content.updates[4].article_id, serviceReliabilityUpdateId);
  assert.equal(homeContent.updates[4].article_id, serviceReliabilityUpdateId);
