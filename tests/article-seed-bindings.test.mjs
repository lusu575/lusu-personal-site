...
const H3_FIRST_VERSION_UPDATE_ID = "seed-update-2026-08-11-h3-first-version-video-sr-48fps";
const H3_FIRST_VERSION_PUBLISHED_AT = "2026-08-11T10:40:00.000Z";
...
const MOBILE_BLOG_RETIRED_PUBLISHED_AT = "2026-09-02T07:20:00.000Z";
const HIDE_MINIMAX_H3_TOOLS_PUBLISHED_AT = "2026-08-13T02:00:00.000Z";
const WALLPAPER_GAME_DISPLAY_UPDATE_ID = "seed-update-2026-08-12-wallpaper-game-display-fix";
const WALLPAPER_GAME_DISPLAY_PUBLISHED_AT = "2026-08-12T07:30:00.000Z";
...
const ARTICLE_SEED_VERSION = "20260908-site-review-r1";
...
function createRecordingD1({ articleSeedVersion = "" } = {}) {
  ...
  function statement(sql, params = null) {
    return {
      ...
      async run() {
        const claimedLease = /^insert into site_runtime_state/i.test(normalizedSql(sql))
          && params?.[0] === "article_seed_version:lease";
        return { success: true, meta: { changes: claimedLease ? 1 : 0 } };
      }
    };
  }
...
test("every article seed D1 binding is defined", async () => {
  ...
  const h3FirstVersionContent = content.updates.find(({ article_id: articleId }) => (
    articleId === H3_FIRST_VERSION_UPDATE_ID
  ));
  assert.ok(h3FirstVersionContent, "the public fallback must include the first-version H3 release");
  const minimaxH3ConsoleContent = content.updates.find(({ article_id: articleId }) => (
    articleId === MINIMAX_H3_CONSOLE_UPDATE_ID
  ));
  assert.ok(minimaxH3ConsoleContent, "the public fallback must include the protected MiniMax H3 console release");
  const hideMinimaxH3ToolsContent = content.updates.find(({ article_id: articleId }) => (
  ...
  assert.equal(content.updates[0]?.article_id, "seed-update-2026-09-08-site-review-optimization");
  assert.deepEqual(
    ...
    [
      ...
      DAILY_AI_NEWS_RSS_UPDATE_ID
    ],
    "Home must project exactly the newest five public updates in release order"
  );
  const mobileBlogRetiredContent = content.updates.find((item) => item.article_id === MOBILE_BLOG_RETIRED_UPDATE_ID);
  assert.equal(mobileBlogRetiredContent.created_at, MOBILE_BLOG_RETIRED_PUBLISHED_AT);
  assert.equal(mobileBlogRetiredContent.updated_at, MOBILE_BLOG_RETIRED_PUBLISHED_AT);
  assert.equal(mobileBlogRetiredContent.published_at, MOBILE_BLOG_RETIRED_PUBLISHED_AT);
  const mobileBlogRetiredSeed = seedBatch.find(({ sql }) => (
    sql.includes(`'${MOBILE_BLOG_RETIRED_UPDATE_ID}'`)
    && /on conflict\(article_id\) do update/i.test(normalizedSql(sql))
  ));
  assert.ok(mobileBlogRetiredSeed, "the mobile Talk retirement metadata must be seeded");
  const mobileBlogRetiredTranslations = boundStatements.filter(({ params }) => (
    params[1] === MOBILE_BLOG_RETIRED_UPDATE_ID
    && ["zh", "en", "ja"].includes(params[2])
  ));
  assert.equal(mobileBlogRetiredTranslations.length, 3);
  for (const { params } of mobileBlogRetiredTranslations) {
    const lang = params[2];
    assert.equal(params[3], mobileBlogRetiredContent.title[lang]);
    assert.equal(params[4], mobileBlogRetiredContent.summary[lang]);
    assert.equal(params[5], mobileBlogRetiredContent.content_markdown[lang]);
    assert.equal(params[6], MOBILE_BLOG_RETIRED_PUBLISHED_AT);
    assert.equal(params[7], MOBILE_BLOG_RETIRED_PUBLISHED_AT);
  }
  const mobileBlogRetiredHome = homeContent.updates.find((item) => item.article_id === MOBILE_BLOG_RETIRED_UPDATE_ID);
  const mobileBlogRetiredProjection = { ...mobileBlogRetiredContent };
  delete mobileBlogRetiredProjection.content_markdown;
  assert.deepEqual(mobileBlogRetiredHome, mobileBlogRetiredProjection);

  const passwordRoomResetContent = content.updates.find(({ article_id: articleId }) => (
    articleId === PASSWORD_ROOM_RESET_UPDATE_ID
  ));
  const passwordRoomResetSeed = seedBatch.find(({ sql }) => (
    sql.includes(`'${PASSWORD_ROOM_RESET_UPDATE_ID}'`)
    && /on conflict\(article_id\) do update/i.test(normalizedSql(sql))
  ));
  assert.ok(passwordRoomResetSeed, "the password-room lifecycle metadata must be seeded");
  const passwordRoomResetTranslations = boundStatements.filter(({ params }) => (
    params[1] === PASSWORD_ROOM_RESET_UPDATE_ID
    && ["zh", "en", "ja"].includes(params[2])
  ));
  assert.equal(passwordRoomResetTranslations.length, 3);
  for (const { params } of passwordRoomResetTranslations) {
    const lang = params[2];
    assert.equal(params[3], passwordRoomResetContent.title[lang]);
    assert.equal(params[4], passwordRoomResetContent.summary[lang]);
    assert.equal(params[5], passwordRoomResetContent.content_markdown[lang]);
  }
  const hideMinimaxH3ToolsHome = homeContent.updates.find(({ article_id: articleId }) => (
    articleId === HIDE_MINIMAX_H3_TOOLS_UPDATE_ID
  ));
  assert.equal(hideMinimaxH3ToolsHome, undefined, "older updates leave the five-item Home projection but remain in the archive");
  for (const item of homeContent.updates) {
    const projection = { ...content.updates.find((entry) => entry.article_id === item.article_id) };
    delete projection.content_markdown;
    assert.deepEqual(item, projection, "Home preserves every non-body field of each recent update");
  }
  assert.equal(hideMinimaxH3ToolsContent.created_at, HIDE_MINIMAX_H3_TOOLS_PUBLISHED_AT);
  assert.equal(hideMinimaxH3ToolsContent.updated_at, HIDE_MINIMAX_H3_TOOLS_PUBLISHED_AT);
  assert.equal(hideMinimaxH3ToolsContent.published_at, HIDE_MINIMAX_H3_TOOLS_PUBLISHED_AT);
  const hideMinimaxH3ToolsSeed = seedBatch.find(({ sql }) => (
    sql.includes(`'${HIDE_MINIMAX_H3_TOOLS_UPDATE_ID}'`)
    && /on conflict\(article_id\) do update/i.test(normalizedSql(sql))
  ));
  assert.ok(hideMinimaxH3ToolsSeed, "the temporary H3 Tools hide metadata must be seeded");
  const hideMinimaxH3ToolsTranslations = boundStatements.filter(({ params }) => (
    params[1] === HIDE_MINIMAX_H3_TOOLS_UPDATE_ID
    && ["zh", "en", "ja"].includes(params[2])
  ));
  assert.equal(hideMinimaxH3ToolsTranslations.length, 3, "the temporary H3 Tools hide release must include three translations");
  for (const { params } of hideMinimaxH3ToolsTranslations) {
    const lang = params[2];
    assert.equal(params[3], hideMinimaxH3ToolsContent.title[lang]);
    assert.equal(params[4], hideMinimaxH3ToolsContent.summary[lang]);
    assert.equal(params[5], hideMinimaxH3ToolsContent.content_markdown[lang]);
    assert.equal(params[6], HIDE_MINIMAX_H3_TOOLS_PUBLISHED_AT);
    assert.equal(params[7], HIDE_MINIMAX_H3_TOOLS_PUBLISHED_AT);
    assert.match(params[5], /admin\/minimax-h3\.html/);
    assert.match(params[5], /Tunnel/);
    assert.match(params[5], /Runner/);
    assert.match(params[5], /GPU canary/);
  }
  assert.equal(
    homeContent.updates.some(({ article_id: articleId }) => articleId === MINIMAX_H3_CONSOLE_UPDATE_ID),
    false,
    "the sixth-newest update must leave the five-item Home projection without leaving the archive"
  );
  assert.equal(minimaxH3ConsoleContent.created_at, MINIMAX_H3_CONSOLE_PUBLISHED_AT);
  assert.equal(minimaxH3ConsoleContent.updated_at, MINIMAX_H3_CONSOLE_PUBLISHED_AT);
  assert.equal(minimaxH3ConsoleContent.published_at, MINIMAX_H3_CONSOLE_PUBLISHED_AT);
  assert.equal(h3FirstVersionContent.slug, "2026-08-11-h3-first-version-video-sr-48fps");
  assert.equal(h3FirstVersionContent.category, "site-updates");
  assert.equal(h3FirstVersionContent.status, "published");
  assert.equal(h3FirstVersionContent.is_pinned, 0);
  assert.equal(h3FirstVersionContent.cover_image, "");
  assert.equal(h3FirstVersionContent.fallbackOnly, true);
  assert.equal(h3FirstVersionContent.date, "2026.08.11");
  assert.equal(h3FirstVersionContent.created_at, H3_FIRST_VERSION_PUBLISHED_AT);
  assert.equal(h3FirstVersionContent.updated_at, H3_FIRST_VERSION_PUBLISHED_AT);
  assert.equal(h3FirstVersionContent.published_at, H3_FIRST_VERSION_PUBLISHED_AT);

  const displayFixContent = content.updates.find(({ article_id: articleId }) => (
    articleId === WALLPAPER_GAME_DISPLAY_UPDATE_ID
  ));
  assert.equal(homeContent.updates.some(({ article_id: articleId }) => articleId === WALLPAPER_GAME_DISPLAY_UPDATE_ID), false);
  assert.equal(displayFixContent.created_at, WALLPAPER_GAME_DISPLAY_PUBLISHED_AT);
  const displayFixTranslations = boundStatements.filter(({ params }) => (
    params[1] === WALLPAPER_GAME_DISPLAY_UPDATE_ID && ["zh", "en", "ja"].includes(params[2])
  ));
  assert.equal(displayFixTranslations.length, 3);
  for (const { params } of displayFixTranslations) {
    const lang = params[2];
    assert.equal(params[3], displayFixContent.title[lang]);
    assert.equal(params[4], displayFixContent.summary[lang]);
    assert.equal(params[5], displayFixContent.content_markdown[lang]);
  }

  const h3FirstVersionSeed = seedBatch.find(({ sql }) => (
    sql.includes(`'${H3_FIRST_VERSION_UPDATE_ID}'`)
    && /on conflict\(article_id\) do update/i.test(normalizedSql(sql))
  ));
  assert.ok(h3FirstVersionSeed, "the first-version H3 metadata must be seeded");
  const h3FirstVersionTranslations = boundStatements.filter(({ params }) => (
    params[1] === H3_FIRST_VERSION_UPDATE_ID
    && ["zh", "en", "ja"].includes(params[2])
  ));
  assert.equal(h3FirstVersionTranslations.length, 3, "the first-version H3 release must include three translations");
  for (const { params } of h3FirstVersionTranslations) {
    const lang = params[2];
    assert.equal(params[3], h3FirstVersionContent.title[lang], `${lang} title must match the first-version H3 fallback`);
    assert.equal(params[4], h3FirstVersionContent.summary[lang], `${lang} summary must match the first-version H3 fallback`);
    assert.equal(params[5], h3FirstVersionContent.content_markdown[lang], `${lang} body must match the first-version H3 fallback`);
    assert.equal(params[6], H3_FIRST_VERSION_PUBLISHED_AT);
    assert.equal(params[7], H3_FIRST_VERSION_PUBLISHED_AT);
    assert.match(params[5], /MiniMax H3/);
    assert.match(params[5], /48fps/);
    assert.match(params[5], /248/);
    assert.match(params[5], /0\.\.62 \+ 61\.\.1/);
    assert.match(params[5], /RealESRGAN_x4plus_anime_6B/);
    assert.match(params[5], /1920×1080/);
    assert.match(params[5], /3840×2160/);
    assert.match(params[5], /BFCache/);
    assert.doesNotMatch(params[5], /random television cameo included|电视机随机 cameo 已加入|テレビ.*cameo.*追加/i);
  }
  ...
  assert.equal(content.updates[10]?.article_id, VIDEO_LINK_AUTOFILL_UPDATE_ID);
  ...
  assert.equal(content.updates[11]?.article_id, H3_AMBIENT_WALLPAPERS_UPDATE_ID);
  assert.equal(
    homeContent.updates.some(({ article_id: articleId }) => articleId === H3_AMBIENT_WALLPAPERS_UPDATE_ID),
    false,
    "the H3 ambient wallpaper release must remain in full history outside the five-item Home projection"
  );
  ...
  assert.equal(content.updates[12]?.article_id, WALLPAPER_SWITCH_SLIM_DAWN_UPDATE_ID);
  assert.equal(
  ...
  assert.equal(content.updates[13]?.article_id, WALLPAPER_SWITCH_CERAMIC_UPDATE_ID);
  assert.equal(
  ...
  assert.equal(content.updates[14]?.article_id, WALLPAPER_SWITCH_CALM_UPDATE_ID);
  assert.equal(
    homeContent.updates.some(({ article_id: articleId }) => articleId === WALLPAPER_SWITCH_CALM_UPDATE_ID),
    false,
    "the calm redesign must remain in full history outside the five-item Home projection"
  );
  ...
  assert.equal(content.updates[15]?.article_id, WALLPAPER_SWITCH_SCENE_UPDATE_ID);
  assert.equal(
    homeContent.updates.some(({ article_id: articleId }) => articleId === WALLPAPER_SWITCH_SCENE_UPDATE_ID),
    false,
    "the scene redesign must remain in full history but outside the five-item Home projection"
  );
