...
- 主站夜间更新汇总与文章浮层整理：
  ...
  - 按参考图追加 10 轮文章页视觉打磨：阅读态知识库窗口沉浸铺满视口，标题栏补最小化/最大化/关闭三按钮，进度条改成底部单行蓝色分段条，并把回到顶部按钮对齐到右侧同一行。
  - 更新 `index.html` 的 CSS / JS query 为 `20260618-article-reference-loop10`，并同步公开 Functions seed、D1 schema seed 和三语 fallback 最近更新。
