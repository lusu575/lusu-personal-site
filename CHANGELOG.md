...
- 将“日本語の裏側”应用 `1.0.4` / 内容 `1.0.3` 候选作为半成品归档到独立 WIP 分支：保留候选代码、题库、10,088 个音频、250 份时间轴、Image2 prompts／管线、14 张 checkpoint WebP 及 62 份 Codex 审核记录，其中 48 份审核证据由清理前工作区补存。
- 新增工具内 [`WIP_STATUS.md`](./tools/japanese-subtext/WIP_STATUS.md)，明确现有图片只作溯源参考、252 份 current-v4 完整资产仍缺 238 份；下一轮默认重新生成并重新审核图片，不沿用旧 PNG 的 review 证据。
- 本次仅保存未完成候选，不合并 `main`、不部署、不修改当前公开应用 `1.0.3` / 内容 `1.0.2`，也不改写公开站点更新记录。
...
- “日本語の裏側”下一候选应用 `1.0.4` / 内容 `1.0.3` 阶段收口：
...
- “日本語の裏側”内容 `1.0.3`（最终应用 `1.0.4`）的前期实现：
  - 全库音频转为本机 AivisSpeech Engine 1.2.0 + 四套 ACML-1.0 AIVMX 日语模型的 CPU 离线预生成链；发布只接受纯假名 reviewed reading、Aivis mora/query/模型来源、无损边界 sidecar、-18 ±1.5 LUFS、最终 MP3 `≤ -2 dBTP` 和实测 `≤ 7.2 mora/s`。
  - 建立全库可朗读任务的 reviewed reading 复核与回归链；“今日”统一锁定为 `きょう`，发音覆盖必须真正体现在锁定假名中，且不得误命中其他汉字复合词的词尾。最终 Aivis 增量生成与媒体验收以发布前实际结果为准。
  - 速率调整升级为 `post-synthesis-active-mora-rate-v3`，以 6.5 mora/s 为目标、校准样本 `≤ 6.6 mora/s`、最多 6 轮收敛，为 Aivis 随机重合成波动留出余量；所有非场景音频的 artifact 与 manifest 均绑定完整策略证据，旧版未调速成品也会失效重录，本地 query 超时和 Windows 瞬时文件占用均纳入有界重试。
  - 清晰度门禁为单 mora 自然迟疑「ん……」增加严格专用速率带：只允许该审校假名使用 `1.2 mora/s` 下限，普通短词仍保持 `1.5 mora/s`，Node、Python 与构建守卫都会拒绝伪造或误套该例外。
  - 全量语音批处理改为关卡级故障隔离：每关开始前用同卷硬链接/备份快照公开媒体目录；单关失败会原子恢复 MP3、scene、timeline 与 manifest，记录完整 audioId 重试清单并继续后续关卡，末尾明确以非零状态和独立 failure report 报告未完成，避免进度无诊断地停在单句或留下错配 checkpoint。
  - 250 张关卡图和两张应用外背景固定通过 OpenAI `gpt-image-2` high 生成；Codex 内置 imagegen 与 Images API 是互斥的真实证据通道，每关 prompt 绑定标题、场景、稳定人物设计、全部台词和题问，禁止 CSS/SVG/程序化图和本地 fallback，并按各通道真实 provenance、prompt/style/source hash、尺寸和像素哈希执行发布门禁。
  - Codex 内置 image2 视觉审核的证据状态改为诚实的 `codex-approved`，保留每份记录原有的 Codex reviewer 标识、六项显式视觉检查、toolRunId 与原始 PNG SHA-256 绑定；导入、发布和构建门禁拒绝误导性的 `human-approved`。
  - image2 发布链新增 Codex 内置 `image_gen.imagegen` 单图导入通道：外部原始 PNG、toolRunId、无裁切归一化结果及 `image2/reviews/<job>.json` 六项 Codex 视觉审核被逐图绑定；发布器同时支持且严格隔离内置工具证据与 Images API request 证据，不伪造 endpoint/request ID。重复出现的场景拓扑或角色身份错误只通过受测的 source-preserving addendum 收紧；例如 L1-017 禁止把安静世界画成决定性目的地，L1-048 保持机器人身份，L1-049 的选择与便笺不得映射答案，L2-005 保持群聊独立端点并禁止复现笑话，L2-012 的资料页禁止伪文字/表格排版。
  - 远程／网络场景的 Image2 prompt 追加“每格为单一不可分割镜头”硬约束，要求用完整四格在两地之间交替，禁止斜线分屏、内部边框、画中画或额外子格；非远程关卡的既有 prompt/hash 保持不变。
  - 对连续生成出远程人物共处、聊天气泡或结论性手势的关卡加入可复现的题面约束 addendum；只重算对应 promptHash，禁止用未入库的临时提示词绕过来源审计。
  ...
  - L2-021～L2-028 补入逐关 source-preserving Image2 约束：保留广播室雨伞/空白名牌、三端工作聊天、美术馆入口相机、三端语音聊天/空白系统日志、高价餐厅点单、便利店工作服/咖啡/电池/空白小票、远程留守电与雨中采访等题面锚点；禁止所有权核验、责任归属、禁拍结论、音量修复、价格/报销、钥匙位置和未来丰收等答案链，以及人物共处、无关道具和提前结局。重建 canonical prompts 后仅这 8 关 `promptHash` 变化，250/250 `sourceTextHash` 不变，选项、正确答案、证据 ID 与解析原文精确泄漏均为 0。
  ...
  - 1.0.3 Image2 manifest 生效后，构建守卫会枚举 `assets/stages/` 顶层并拒绝遗留的 `manifest.json` 或 `l[1-5]-NNN.webp`，防止旧配图随新版重复发布和占用空间。
  - 桌面端和手机端关卡布局收紧，完善首次模式选择、无自动播放、声音失败文字回退、结果/解析/下一关流程、不强制滚动和打卡式学习记录；手机端关键状态/提示文字保持 16px 可读基线，模式弹窗被阻止关闭时只聚焦仍可用的入口。
  - 结果弹窗改为必须选择“重新挑战 / 查看解析 / 下一关”的任务态，Esc 与遮罩点击不再把已提交题目留在死路；选项结果增加固定尺寸 ✓/× 可见标记而不塞入“正确答案”文字，单文件音频重试也会恢复原句、词块或选项，不再一律跳回整场。
  - 工具页面、六个嵌套 ESM 与主站入口缓存键统一升级为 `20260712-japanese-subtext-v103-r6`，并把三个发布门槛重新绑定 r6 合同；主站 Resources 入口会携带当前界面语言，844×390 等手机横屏继续使用桌面横版背景，只有 900px 以内竖屏切换手机竖版背景。
  - 云存档将 250 关 + 5,000 条活动的进度同步从 5,256 条 D1 查询收敛为固定 8 条；5 条写入位于同一原子 batch，拒绝超前时间和矛盾打卡，并修复旧未来时间/孤儿记录。
  - 独立工具维护文档和 `CHANGELOG.md` 锁定每次公开维护 `+0.0.1`；网站上线、1.0.1、1.0.2 与候选 1.0.3 更新文章分开追加，不回写已发布记录。
  - 上线、1.0.1 与候选 1.0.3 三语文章新增标题、摘要、正文 SHA-256 指纹守卫，与既有 1.0.2 一样拒绝同步回写 fallback / Functions / schema 后绕过构建；候选文章只在版本门槛完成并部署后成为公开记录。
  - 补齐工具 README、维护手册与独立变更日志的交叉链接及 image2 发布/迁移命令；同时把尚未发布的上线与 1.0.1 三语种子时间统一校正到上海时区 2026-07-11，fallback、Functions、schema 与构建守卫保持一致。
  - 将发布报告的 AUDIO / IMAGE2 / BROWSER 三个 PENDING 门槛绑定当前 Aivis／Image2 契约哈希；最终资产和回归完成前继续保持 PENDING，禁止复用旧合同标记伪装通过。
  - 正式 release-check 新增 Python TTS unittest 入口；通过临时 `JP_SUBTEXT_TTS_PYTHON` 选择隔离解释器，避免只跑 Node 门禁而漏掉语速解析、原子发布和回滚回归。
  - L3-001～L3-010 补入逐关 source-preserving Image2 约束：固定实体审查室、教室、餐馆、山间旅馆、四端群聊、同一虚拟战场/休息室、非人 AI 终端、探索船日志审查区与同一实体档案室等题面拓扑；禁止资金/批改/称呼/时间/消息选择/战斗时序/声画同步/假设映射/无人机证据链和清洁人员推理链直接泄题。L3-009/010 的 `log` 不再误触远程布局，`船長`、`回収用ドローン` 等已存在于 canonical source 的选项短语被显式分类为 source collision，而非生成器泄漏；重建后仅这 10 关 `promptHash` 变化，250/250 `sourceTextHash` 不变，真实额外选项、正确答案与解析插入仍为 0。
  - L3-011～L3-015 补入逐关 source-preserving Image2 约束：锁定文化祭委员会、提案会、餐后同桌、酒店前台和深夜聊天两端的题面拓扑与等权空白证据；禁止投票/责任归属、内部采用结果、代付结论、换房补偿完成态和已读时序链直接泄题。L3-013 的“通信量”不再误触远程布局；重建后仅这 5 关 `promptHash` 变化，250/250 `sourceTextHash` 不变，真实选项/解析泄漏与 canonical source collision 均为 0。
  - 对已完成的 L1 成图做第二轮不依赖审核 JSON 的逐图复审；L1-007、L1-010、L1-018、L1-019 因“未实际离线”、“新鲜面包提前出炉”、心虚漫画符号和非人终端被拟人化而撤回通过。四关新增受测的 source-preserving addendum，只改对应 `promptHash`，题库和 250/250 `sourceTextHash` 保持不变；旧图在 IMAGE2 重生与复审前不得发布。
  - L1 成图复审继续撤回 L1-029、L1-030、L1-031、L1-032、L1-035、L1-036：禁止把游戏系统实体化或用猫的满足动作画死因果，锁定考试教室逐步转为深海的时序，清理海报/投影/售货机伪文字，并禁止远程手机叠层和重复设备。六关受测 addendum 已重建 canonical prompt，旧图在 IMAGE2 重生前自动因 `promptHash` 过期而失效。
  - L1 最后十关复审撤回 L1-046、L1-047、L1-049、L1-050：锁定猫两次推动同一只杯子的物理连续性、借书只存在于借阅者远程端点、清除开发屏幕/便笺伪代码，并移除题外墙钟，确保带表带的腕表和“第二杯放到对面空座”动作连续。四关旧 raw 因新 `promptHash` 失效，L1-048 继续通过机器人身份与空白屏幕复审。
  - L3-016～L3-020 新增逐关 source-preserving Image2 约束：固定两端留言、直播制作环境、校内非人扬声器、非视觉旁白和梦中车站拓扑；L3-018 不再被误判成远程通信，并禁止用碎片匹配、广播利益链、火灾撤离、循环时间线或“右侧”图形把题面结论再画实。
  - L3-021～L3-030 新增逐关 source-preserving Image2 约束：固定占卜馆未拆信封、谒见厅空白账簿、当代条约展、同一录音控制环境、闭馆图书馆、电梯、本地室内询问、事后事故听取、同一森林任务空间与姐弟书桌，禁止把比喻、文书、数量、波形、楼层/版面位置、湿伞证据、新闻记忆、任务改写及答案纸用途拼成视觉答案链。L3-024 的 diagnostic log、L3-027 的 access log 与 L3-029 的 upper stream 不再误触远程布局；L3-025 的蓝色发光札按黑白合同仅保留微弱灰阶光，L3-030 停在背面成为画纸前并禁止“翻面→空白背面→宇宙船作画”完整链。canonical 重建只改变这 10 关 `promptHash`，250/250 `sourceTextHash` 不变，真实选项/解析泄漏与 canonical source collision 均为 0；`hero` / `reporter` 既有 cast-id 别名仅作为锁定题库数据风险记录，本批不改题库、ID 或双语姓名。
  - L3-031～L3-040 新增逐关 source-preserving Image2 约束：锁定职场评估、重逢、餐厅试味、工坊参观、四端群聊、本人／预测 Avatar、会后纪要终端、单人飞船日志、记者会及深夜电台的题面拓扑与等权空白证据，禁止把数量、日期、责任、转述链、聊天操作、时序、投票、暗号和取消结论画成答案链。L3-038 的飞船本地硬件／日志不再误判为远程通信；L3-036 的 Predictive Avatar 只能显示在虚拟屏幕且继承 Yui 的脸部与发型特征，不得成为第三名实体人物。canonical 重建仅改变这 10 关 `promptHash`，250/250 `sourceTextHash` 不变，真实选项、正确答案、解析泄漏与 canonical source collision 均为 0；不改题库或 L3-041 以后关卡。
  - L3-041～L3-050 新增逐关 source-preserving Image2 约束：锁定单册重装日记、末班车后的遗失物室、火车告别与非人格字幕层、辩论台／控制室、龙窟茶点、旋转居住区／非人引导 AI、三名旅客与一名服务员同桌、三人会议／缺席部长电话端、循环早晨的纸张和会话 AI 麦克风灯等题面拓扑与等权空白证据；禁止把日记时序、未来遗失物、字幕警告、播出控制、停战、参照系、生日惊喜、借用权威、剩余尝试和监听判断画成答案链。L3-043 的字幕层不再误判为远程通信；L3-046 的通用 `guide`、L3-048 的 `boss` 与 L3-050 的 `ai` 稳定人物卡均由本关来源角色／物种覆盖，不能带入人类导游、奇幻首领或拟人 AI。L3-047 保留 revision 5 的“三名旅客与一名服务员”场景；canonical 重建仅改变这 10 关 `promptHash`，除 L3-047 因题面来源修订更新 `sourceTextHash` 外其余 249 关保持不变，真实选项、正确答案、解析泄漏与 canonical source collision 均为 0；不改 L4 以后关卡。
  - L4-001～L4-010 新增逐关 source-preserving Image2 约束：锁定实体预算会、借用终端／讲台两拍、四人桌与缺席来客、同一机场登机口、同一 VRChat 调解室、四端联机游戏、逐一分开的访谈、采访／制作／播出媒体流水线、本地 AI 评审室和两艘船／管制三端通信；禁止把预算拒绝、账号操作者、生日惊喜、故意滞留、现实身份、私聊泄露、证词串联、责任消隐、职级偏见或代词指称画成答案链。L4-003/004/008/009 明确走本地布局，L4-010 由显式 remote override 固定三处整格交替，L4-002/005/006 保持远程；L4-008 使用专属媒体流水线而非通用远程模板。通用 `sales`/`teacher`/`avatar-*`/`ai` 等 cast id 必须服从本关来源角色与非人身份，不改系统性 designId；L4-003 保留 revision 5 对题外 Saya 的修正。重建后仅本批 `promptHash` 与已修正文案对应的来源哈希按实际内容更新，真实选项、正确答案、解析泄漏及 canonical source collision 均为 0；不改 L4-011 以后关卡。
  - L4-011～L4-020 新增逐关 source-preserving Image2 约束：锁定四端活动群聊、三端异步留言、校内广播两端、四端工作邮件、创意餐厅、本地旅行团讨论、人类／精灵议会、当前证词复盘、实体条约听证和同一舰载 AI 审计室；禁止把时序、遗漏回复、广播暗号、邮件升级、菜品责任、定位控制、议会期限、鞋子动机、翻译责任及记忆删除归因画成答案链。L4-020 的舰载日志不再误判为远程场景。同步修正 L4-011 简中姓名字形、L4-012 日文解析混词、L4-014 英文称谓／问句与日文解析、L4-018 中文标题提前泄露“故意”；答案逻辑和日语发音保持不变，source projection 只在实际涉及的关卡更新。
  - L4-031～L4-040 新增逐关 source-preserving Image2 约束：锁定选举播报／观众端、救援船返航清单、两名现实用户与单一共享 Avatar、远程会议／非人纪要 AI、碎纸证据、雨中旅行团集合点、龙族道歉档案／非人法庭 AI、政府新闻播报端、单份私人文档复盘和游戏内角色／外部记录层的题面拓扑；禁止把统计分母、救援服使用者、Avatar 操作者、断线表决、纸张来源、雨伞归属、道歉真伪、网络沿革、文档体裁或双门选择画成答案链。L4-032/037 使用显式 local override，L4-033 使用显式 remote override 并固定为一个共享 Avatar，L4-040 的 Choice UI 纳入非人系统约束；同步保留 L4-032 中文返航报告、L4-034 掉线者判断／解析与发音、L4-040 中文题问的定向修正。实时计算测试确认 10 关真实选项／解析泄漏为 0、`sourceTextHash` 不变；canonical v4 已在 2026-07-14 随全库任务重建，raw 与 review 仍须按新哈希重生／复审后才能发布。
