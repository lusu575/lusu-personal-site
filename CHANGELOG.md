...
- 管理后台视频置顶排序字段联动优化：视频管理表单未勾选置顶时会禁用置顶排序输入框并显示提示，勾选置顶后再启用并自动填入下一个置顶排序，减少误编辑；同步更新后台私有更新记录和后台 JS query。

- 管理后台新建视频置顶排序提示优化：新建视频未勾选置顶时，置顶排序字段保持空白；勾选置顶后再自动填入下一个置顶排序，取消勾选会清空该字段，减少普通排序和置顶排序混淆；同步更新后台私有更新记录和后台 JS query。

- 管理后台聊天室刷新丢失选中记录提示：聊天室消息刷新后如果当前选中记录已不在最新列表中，会清空编辑表单并显示提示，避免继续处理旧聊天记录；同步更新后台私有更新记录和后台 JS query。

- 管理后台账号列表刷新丢失选中项提示：账号列表刷新后如果当前选中账号已不在最新列表中，会清空账号编辑区并显示提示，避免管理员误以为旧账号仍可编辑；同步更新后台私有更新记录和后台 JS query。

- 管理后台文章列表刷新丢失选中项防护：知识库文章刷新后如果当前选中文章已不在最新列表中，会自动清空编辑表单并提示，避免继续保存已不存在的文章；同步更新后台私有更新记录和后台 JS query。

- 管理后台视频列表刷新丢失选中项防护：视频或视频分类刷新后如果当前选中项已不在最新列表中，会自动清空对应编辑表单并提示，避免继续保存或删除已不存在的对象；同步更新后台私有更新记录和后台 JS query。

- 管理后台聊天室消息读取期间锁定编辑：聊天室管理刷新消息列表时，会临时锁定消息列表、编辑表单和保存/隐藏/删除/禁言按钮，避免列表读取中继续处理旧记录；同步更新后台私有更新记录和后台 JS query。

- 管理后台视频元数据读取期间锁定编辑：视频管理自动识别链接或刷新元数据请求进行中会临时锁定视频表单、保存/发布/删除按钮、视频列表切换和新建入口，并在刷新元数据响应返回时校验当前选中视频，避免慢请求覆盖另一个视频；同步更新后台私有更新记录和后台 JS query。

- 管理后台账号详情读取期间锁定表单：切换到已有账号但详情尚未读取完成时，会先清空旧表单并临时锁定邮箱、角色、新密码和密码状态字段；详情读取失败后继续保持空表单锁定，避免加载中或失败态输入内容随后被接口返回覆盖；同步更新后台私有更新记录和后台 JS query。

- 管理后台文章详情读取期间锁定表单：切换到已有知识库文章但详情尚未读取完成时，会临时锁定编辑表单和语言切换按钮；详情读取失败后继续保持表单锁定，避免加载中或失败态输入内容随后被接口返回覆盖；同步更新后台私有更新记录和后台 JS query。

- 管理后台文章写入期间锁定表单：知识库文章保存、发布或删除请求进行中会临时锁定 slug、分类、封面、发布时间、置顶和三语标题/正文等字段，并同步禁用语言切换按钮；同步更新后台私有更新记录和后台 JS query。

- 管理后台视频写入期间锁定表单：视频保存、发布或删除请求进行中会临时锁定链接、标题、封面、分类、排序和发布状态等字段，并同步禁用自动识别、刷新元数据和清空封面入口；同步更新后台私有更新记录和后台 JS query。

- 管理后台视频分类写入期间锁定表单：视频分类保存或删除请求进行中会临时锁定 slug、分类名称、排序和启用状态字段，避免写入尚未完成时继续修改内容造成提交范围误解；同步更新后台私有更新记录和后台 JS query。

- 管理后台账号保存期间锁定表单：账号管理保存邮箱、角色或重置密码请求进行中会临时锁定账号表单字段，避免保存尚未完成时继续修改内容造成提交范围误解；同步更新后台私有更新记录和后台 JS query。

- 管理后台聊天治理期间锁定表单：聊天室保存、隐藏、删除或禁言请求进行中会临时锁定昵称、内容和禁言参数输入框，避免慢请求期间继续修改字段造成误以为已经一并提交；同步更新后台私有更新记录和后台 JS query。

- 管理后台聊天禁言同步列表状态：从聊天室记录发起禁言用户 ID 或 IP 来源时，禁言列表会同步进入刷新中状态并临时禁用刷新、停用按钮，避免禁言写入与列表操作并发导致状态闪动；同步更新后台私有更新记录和后台 JS query。

- 管理后台聊天筛选读取防抖：聊天室消息列表读取中会临时禁用“显示隐藏”筛选，并忽略重复读取入口，减少快速切换筛选造成的重复请求和列表闪动；同步更新后台私有更新记录和后台 JS query。
