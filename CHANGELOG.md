...
- 修复临时互传在 Cloudflare Pages 缺少 `TRANSFER_BUCKET` 时仍创建假上传任务的问题：客户端在 R2 未就绪时直接禁用文件选择与投放，不再显示 100% 后才统一报错；服务端保留稳定诊断码。正式文件上传仍须先在 Cloudflare 创建私有 `lusu-temp-transfer` 并绑定，根配置不会引用不存在的资源而阻塞整站部署，Preview 也不会复用正式桶。
