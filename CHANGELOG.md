...
- 修复临时互传文件上传在 Cloudflare Pages 缺少 `TRANSFER_BUCKET` 时统一报“服务暂时不可用”的部署缺口：根 `wrangler.jsonc` 现在分别声明 Production 与 `env.preview` 私有 R2 桶，并完整保留 Preview 的非继承配置，构建会阻止绑定再次遗漏或预览部署误用正式桶；客户端在 R2 未就绪时不再创建显示 100% 后失败的假上传任务。
