...
- 管理后台实时大屏地图升级为最近 14 天的城市级精确聚合视图：桌面支持鼠标滚轮缩放、拖拽、点击和悬停，触屏支持双指缩放与拖动，键盘可聚焦点位并查看城市 PV/UV。
- 地图点位和控件使用至少 44px 触控目标，并保留同数据城市列表；详情不展示 IP、网络前缀或隐藏标识等敏感数据。后台资源 query 更新为 `20260716-admin-interactive-map-r1`。
