...
- 修复手机端临时互传发送区以 sticky 层覆盖已发送图片和文件卡片的错位；composer 回到单一房间滚动容器的正常文档流，发送后图片改为全宽稳定预览框，文件信息卡同步占满可用宽度。
- 构建检查读取文本时统一 CRLF / LF 换行，避免 Windows 工作区因后台 SVG `viewBox` 的多行守卫产生误报；知识库自动焦点与互传正常流布局也已加入静态回归守卫。
...
- 新增临时互传客户端 UI 合约测试与移动文章构建守卫；公开 `site-updates` 记录使用 `seed-update-2026-07-17-mobile-transfer-send-fix`，已同步 `js/main.js` fallback、Functions seed 与 schema seed，相关主站资源 query 更新为 `20260717-mobile-transfer-send-r2`。
