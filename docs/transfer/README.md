...
1.0.12 起，房间过期或管理员删除都是不备份的彻底删除：系统会中止未完成 Multipart，清除 R2 文件和 D1 内的条目、会话与房间记录。完成后同一密码会创建全新空房；部分存储删除失败时房间保持锁定并由后台、清理任务或下次加入重试。手机上的“发送图片／发送文件”每次都重新备战系统 picker，取消、权限拒绝或选错来源后可立即重试。

1.0.14 起，未发送文字与附件只属于当前房间，离房、关闭互传或退出登录时清空，绝不写入浏览器存储或历史记录。切换浏览器标签页只停止被动列表轮询，不再主动暂停已提交上传或打断分片重试；浏览器自身仍可能冻结后台页面。返回前台后刷新内容和进度，用户的手动暂停保持有效。真正离开互传仍按原规则停止任务，并仅保留可恢复 Multipart 元数据，重新进入后需重选同一文件。

1.0.13 优化独立管理页：与主后台共用阅读与颜色规范，容量配额支持 MiB / GiB 精确输入和范围校验，通知配置、接收端响应与失败记录使用真实状态。公开侧仅按子项目治理规则同步可见版本，房间、文件、配额默认值与协议未变。
...
- Quick Transfer 因共享能力注册表加入浏览器游戏接管候选、公开视频读取与站长视频外链记录管理候选而按治理规则升至 1.0.9；这是共享入口的治理升版，互传房间、口令派生、AES-GCM 文字、私有 R2 文件、配额、Multipart、鉴权与发布完成后 24 小时生命周期保持不变，`games:play` 与视频／内容 scope 也不能访问任何 Transfer 数据。
- Quick Transfer 1.0.10 只精修公开入口与独立后台的交互反馈：实际请求按钮独享 busy／`aria-busy` 和 spinner，复制反馈只覆盖真实剪贴板请求并短时复原，notice 保留稳定槽位，后台表格与对话框支持可中断进入／退出。协议、口令派生、AES-GCM、私有 R2、滚动配额、Multipart、鉴权和发布完成后 24 小时生命周期均未改变。
- Quick Transfer 1.0.11 is a governance-only patch: the shared Agent Auth registry now carries the owner-only `minimax-h3:execute` scope. Transfer protocol, encryption, private R2, quota, authentication, and 24-hour expiry semantics are unchanged.
...
- CLI 与 stdio MCP 下载都默认拒绝覆盖已有文件；stdio MCP 的上传和下载还必须位于显式允许的目录根内。访问令牌、本地房间状态和环境变量都属于敏感本机数据，不得提交、打印到日志或传入 MCP 工具输出。公开只读远程 MCP 与站长知识库远程 MCP 都不开放远程互传；Quick Transfer 仍只有站点 API、本地 CLI 和本地 stdio MCP 能力面。
...
未配置 webhook 时，后台仍保存站内告警并明确显示“站内通知尚未配置”，自动展开配置步骤，测试按钮保持禁用。仅配置 `TRANSFER_ALERT_EMAIL` 不会发送邮件；该字段作为接收服务的转发目标，真正发送由接收服务负责。

测试通知只由管理员手动触发，并先说明会对外发送。界面根据返回的 `delivered` 区分接收端已接收、发送失败和未配置；HTTP 200 或记录创建成功不能当作通知已送达，更不能证明邮件已经送达。官方预算提醒状态仍需在 Cloudflare 核对，本站无法读取。

后台容量输入在 MiB / GiB 之间切换时按整数个字节精确换算，单位切换不会产生配额变动或未保存提示。原有任意整数字节设置可无损回读；不能精确换算的输入会被拒绝。提交字段名、数值单位（字节）、服务端范围与 `expectedUpdatedAt` 并发保护均保留。
...
- 草稿与异步隔离：A 房草稿／附件离房后不可出现在 B 房；旧文字发送、配置回读、加入房间、上传完成不得修改新房间，关闭后未发送明文不得留在 DOM 或持久存储。
- 入口恢复：首次加载模块失败后按钮可重试；并发点击只启动一次，切语言或重绘卡片不解除启动中的禁用状态，离开再返回时旧请求不能再次打开工具。外层模块与内部样式／片段／脚本失败分别由各自入口展示重试。
- 后台上传：普通 XHR、Multipart 初始化／分片／重试／完成期间切 hidden→visible 不产生主动 abort 或重复任务；队列继续按有界并发执行，手动暂停后保持暂停，快速暂停→继续及离房后的 Multipart 恢复仍可用。
