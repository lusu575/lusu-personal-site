...
- Quick Transfer 因共享能力注册表、CLI、stdio MCP 及其定向测试被 Life Restart 游戏 Agent 接入命中而按治理规则升至 1.0.7；互传房间、口令派生、AES-GCM 文字、私有 R2 文件、配额、Multipart、鉴权与发布完成后 24 小时生命周期保持不变，`games:play` 也不能访问任何 Transfer 数据。
