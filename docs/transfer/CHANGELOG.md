...
- 独立管理页统一 Neo-XP 色彩、字号、卡片网格、窄屏表单和可键盘访问的帮助说明；主后台保护、图集来源、分页与危险操作确认保持不变。
- 普通用户容量支持 MiB / GiB 输入与无损切换，整数个字节精确换算并按既有服务端范围校验；仍提交原字段与字节值，保留 dirty、版本冲突与草稿保护。
- 费用卡片改用中文解释 Class A / B、存储月累计与普通池状态；补全通知接收服务配置步骤，区分地址已配置、接收端已接收、发送失败与仅站内记录，不把测试记录创建成功当作通知或邮件送达。未配置时禁用发送测试，未进行线上测试通知或环境变量修改。
- 公开界面仅按子项目强制升版规则同步 v1.0.13 及片段缓存；公开互传逻辑、配额默认值、私有 R2、Multipart、鉴权与 24 小时生命周期未改变。
...
- 键盘、Escape、`prefers-reduced-motion` 与站内 motion-off 会立即或无位移完成关键状态；hover 仅在支持 hover 的细指针设备启用。本次从 1.0.9 精确升至 1.0.10，互传房间、口令派生、AES-GCM 文字、私有 R2 文件、普通账号滚动 24 小时配额、Multipart、鉴权和发布完成后 24 小时过期语义均未改变。

## 1.0.9 - 2026-08-09

- 共享能力注册表加入浏览器游戏配对／观察／语义动作／暂停／关闭候选，并补充公开视频读取与站长视频外链记录管理候选；这些候选继续按各自 scope、部署和生产验收边界管理。
- 本次修改命中 Quick Transfer 的共享受管入口，因此从 1.0.8 精确升至 1.0.9；没有改变互传房间、口令派生、AES-GCM 文字、私有 R2 文件、普通账号滚动 24 小时配额、Multipart、鉴权或发布完成后 24 小时过期语义，游戏与视频能力也不能访问 Transfer 数据。
...
- 本次修改命中 Quick Transfer 的共享受管入口，因此从 1.0.6 精确升至 1.0.7；没有改变互传房间、口令派生、AES-GCM 文字、私有 R2 文件、普通账号滚动 24 小时配额、Multipart、鉴权或发布完成后 24 小时过期语义，游戏能力也不能访问 Transfer 数据。
