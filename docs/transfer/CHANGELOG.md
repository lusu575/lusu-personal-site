...
- 未发送文字与附件在离房、关闭互传和退出登录时清空；三语输入提示明确当前房间的草稿边界，不把未发送文字、口令或密钥写入持久存储。
- 切换浏览器标签页只暂停被动列表轮询和不可见 UI 刷新，保留用户提交上传的网络请求、重试计时器和有界队列；恢复前台后更新列表与进度，不改变用户手动暂停状态。
- 修复快速暂停后立即继续的队列竞态、上传成功后房间 generation 字段不匹配导致列表未回读，以及旧配置／加入房间结果跨房间或关闭后污染状态的问题。
- 恢复任务只合并未过期 Multipart 元数据，访问其他房间不再覆盖先前房间的续传记录；工具入口模块加载失败时显示可重试状态，并在并发点击、语言切换、重新渲染与离开再返回时保持当前启动状态。
- 离开互传后的取消与 Multipart 元数据恢复、幂等键、AES-GCM、私有 R2、配额、鉴权和发布完成后 24 小时过期继续沿用既有契约。
