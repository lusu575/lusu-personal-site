...
本目录是在线画板的治理根：`VERSION` / `project.json` 保存独立版本，`CHANGELOG.md` 保存子项目更新，`AGENTS.md` 约束维护流程，`AGENT.md` 仅作兼容入口。任何画板更改都必须把该版本精确增加 `0.0.1`，并同步本目录全部受影响文档和根项目记录。1.0.7 补齐 Pages 主 mutation gate 对精确 Agent Yjs scene POST 的媒体类型例外；与 1.0.6 的精确 raster 入口修复一样，同源检查仍先执行，完整 Agent 与房间鉴权、正文和只追加场景校验均未放宽，其他来源、方法、路径与 MIME 继续失败关闭。既有元素／资源仍不可修改或删除，浏览器协作协议与房间生命周期保持兼容。
