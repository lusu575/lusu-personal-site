...
本目录是在线画板的治理根：`VERSION` / `project.json` 保存独立版本，`CHANGELOG.md` 保存子项目更新，`AGENTS.md` 约束维护流程，`AGENT.md` 仅作兼容入口。任何画板更改都必须把该版本精确增加 `0.0.1`，并同步本目录全部受影响文档和根项目记录。1.0.10 将大厅收敛为紧凑匿名身份栏、公共／密码房并列入口和最近使用行，并修复密码说明在鼠标点击后常驻的问题。继续复用已有 Image2 素材与 XP 控件；画布外观、DO 协议、同步和 24 小时生命周期不变。

在线画板是鲁肃个人站**工具区**中的独立工具页：`/tools/whiteboard/`。它不新增或恢复“资源区”页面、入口、路由或分类；主站内部继续保留 `resources` 兼容键，用户可见文字统一为“工具区 / Tools / ツール”。

## 大厅交互

- 匿名名字只显示一处标签，改名和冷却状态留在同一紧凑身份栏；身份加载与失败有明确文案和重试操作。
- 两种入房方式共享卡片层级、44px 最小按钮和 XP 立体边框。公共画板明确公开共享；密码房说明与输入分层。最近使用为空时只占紧凑一行，有密码房记录时可直接把焦点移到密码输入框，不读取或保存旧密码。
- 密码说明以原生按钮控制：鼠标悬浮显示，离开按钮立即关闭，鼠标点击不能固定打开；键盘聚焦显示，Escape 可关闭且焦点仍留在按钮；触屏／笔触点击切换，点击外部或焦点离开时关闭。
- 说明始终位于正常文档流中，展开时为正文留出空间，不覆盖密码输入或主操作；不使用 `details[open]` 或 `:focus-within` 绕过受控关闭状态。所有文案保持 zh／en／ja。
