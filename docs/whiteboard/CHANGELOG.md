...
- 大厅改为紧凑匿名身份栏、公共／密码房两个同层级入口与底部最近使用行；删除重复昵称标题，压缩空记录占位，统一 XP 浅纸按钮、立体边框、标题和操作落点，继续复用已有 Image2 背景与图标。
- 密码帮助改为受控原生按钮：鼠标移开即隐藏且点击不常驻；键盘聚焦展示、Escape 关闭；触屏／笔触点击切换并支持外部关闭。说明在正常流中展开，不再遮挡密码输入和提交按钮。
- 补齐三语匿名身份加载、公共画布保留提示和最近密码房重新输入操作，密码错误与输入框通过 ARIA 关联，所有大厅操作保持至少 44px 触控尺寸。
- 本次只调整大厅与帮助交互，不修改 Excalidraw 画布、Yjs／WebSocket 协议、Worker、匿名身份服务、图片权限或 24 小时生命周期。
