...
- `rate_limited`、同步预算和普通网络中断是可恢复状态；访问拒绝、无效票据和协议破坏才可停止自动重连。用户提示必须保留具体类别，不能全部退化成“发生了错误”。
...
- 默认画布使用暖纸底、石墨灰、hachure 与高 roughness 的铅笔草图风，同时保留 Excalidraw 工具能力和本站 XP/Pixel/Y2K 外壳。
