...
- Pages Functions 的文件路由依赖 Cloudflare Pages Production 中名为 `TRANSFER_BUCKET` 的私有 R2 binding；目标桶为 `lusu-temp-transfer`。线上 `TRANSFER_R2_NOT_BOUND` 与配置级部署失败共同说明目标桶尚未创建或当前 Pages 项目无权绑定；根 `wrangler.jsonc` 在资源存在前不得引用它，以免阻塞整站发布。创建桶并绑定后必须重新部署，再以匿名文件路由由 503 变 401、登录配置 `r2Ready: true` 和真实上传闭环验收。Preview 未配置独立桶时保持文件上传关闭，绝不复用正式桶。
