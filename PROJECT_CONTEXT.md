...
- 城市点位和地图控件的触控目标至少 44px；地图详情与同数据列表不显示 IP、网络前缀、visitor id、hash 或其他隐藏标识。
- 本轮仅更新管理后台私有界面与后台文档，不进入公开 `site-updates`；后台资源 query 为 `20260716-admin-interactive-map-r1`，维护细节见 `admin/docs/ADMIN_PROJECT_CONTEXT.md`。
