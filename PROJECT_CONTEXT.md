...
- 采集阶段的 query provenance 与 direct-source provenance 是两套独立权限。`queryIds`／`mustReviewQueryIds` 来自原始 Google News topic items，或公开 X profile 配置中显式声明且已按当次权威查询目录重新对齐的 `queryId`；公开 X、RSS 等直源的 `mustReviewSourceIds` 与 source review lane 独立保留。公开 X 抓取器不得自行把关联 query 硬编码为 required／must-review：Tibo 的 required query 必须留下 must-review query 归属，Mistral 等 supplemental query 不得被 direct-source 权限反向提升。
...
- 这是必需查询结构性过宽而不是网络、候选不足或发布权限故障：Cybercab 与 Wayve／Uber 在同一天形成两个高转载量事件，按公司名做第一层拆分仍分别超过 99 条。修复保留原宽查询作为 supplemental，并以直源／可靠媒体、运营商、监管和排除热点后的通用部署七条 required 分片承担完整性签收；不得通过提高结果上限、把截断伪装为空结果或删除自动驾驶范围规避门禁。
- 每次新增或修改高流量 required 查询，都应先用目标精确窗口执行 max-plus-one 实探。任何分片一旦触发第 100 条，继续按实体、事件阶段、来源类别或动作意图缩窄，同时保留宽父查询作补充；回归测试固定 broad supplemental 与 required／must-review 分片清单，避免未来配置回退。
- 同日公开 X 页面不再提供之前的 schema.org `itemID`／meta，而把规范 tweet ID、`full_text` 和 `created_at_ms` 放进 React relay 记录；旧解析器因为 `<article>` 仍存在而把所有账号错误记为 `success + 0`。新解析器只接受在当前 profile DOM 中出现的同账号 status ID，再从不执行的 relay 文本中提取该 ID 对应正文与毫秒时间，避免误收引用帖；文章存在却没有任何可验证记录时必须失败，不能再把结构漂移当空结果。真实页面回放 Tibo 5 条可解析、4 条位于原精确窗口。
- 第三个根因是客观预筛只豁免 editorial signal 与 RSS，导致已经抓到但标题不含固定变化词的公开 X 直达帖被程序标为低信号，没有进入 Codex 队列。公开 X 候选现在无条件绕过程序预筛，队列同时保留 `mustReviewQueryIds` 与 `mustReviewSourceIds`，保证 Tibo、OpenAI、OpenCode、Qwen、LTX、Runway 等指定账号的每条返回线索都由 Codex 实际审阅。
- 站长将时间规则改为连续采集锚点：定时器仍每天 07:00 触发，但从 2026-09-04 起正式窗口为 `[前一报告日已保存的采集启动时刻, 本次执行实际采集启动时刻)`，不再强制恰好 24 小时。每次同日重跑或人工恢复都用当前 Horizon run 替换本报告日锚点并向后推进右边界，左边界仍使用上一报告日锚点；candidate index、coverage manifest、组装记录、validator 与人工恢复会交叉核对 v2 策略、两端时间和当前 run id。上一日锚点不存在时失败关闭。
- 第四个根因是语义队列曾只按规范化标题合并引用；同一标题被不同 required 通道命中时可能拥有不兼容的 editorial signal 集合，令一个 Codex 判断无法同时通过各引用的 finalize 分类约束。聚类身份现加入完整信号合约，保留相同标题但不同受保护语义的独立审阅组。
...
- 检测到较新的云存档时，`games/game-shell.js` 必须立即停止 10 分钟自动同步和隐藏页／退出／导入等全部上传路径，并显示三语 XP 冲突窗口。用户可以先下载本地 JSON 备份，再明确恢复云端、用本地覆盖当前云端版本或暂不处理；取消、Escape 和外点都只暂停，不得暗中上传。覆盖动作仍受服务端版本校验保护；恢复云端前必须重新 GET 并核对仍是弹窗所示版本，变化时不得应用旧快照。
