...
- 工具雷达以每个工具一张有语义的说明图为正常目标、最多两张；选图与截图必须按 `自动新闻/integrations/lusu-site/tool-radar/VISUAL_METHOD.md` 执行，而不是每期临时发挥。取图前先写视觉任务卡，明确“读者问题、要证明的结论、必须出现的 2–5 个元素、单图或双图关系与三语图注规划”；单图连同图注必须独立说清工具／功能、动作或步骤及结果／价值，双图只能组成输入／输出、操作／结果、前后或连续步骤，并保持同组、明确先后和互补角色。关键标题、输入、动作、输出与结果不得被导航或裁切截断；逐张通过脱离正文的三秒测试、正文宽度可读性、版权、隐私、稳定性和本地资产检查后才可进入运行记录与三语正文。失败时只能按同一任务重截、改用基于已核实事实的本站原创说明图，或从三语稿一起删除；不能用空界面、泛化 Hero 或随意裁切填版面。公开素材优先使用本站原创可复现演示、明确许可的官方素材或本站原创说明图；临时官网审稿截图不等于获得正式转载权。正式入库仍须保存来源、权利依据、核对日期、SHA-256、三语 alt 与图注，并在投递前确认 Pages 线上资源可读取且远程 SHA-256 与审阅文件一致。
...
- 管理后台“自动投递”页在 `daily-ai-news` 与 `tool-radar` 间切换，分别显示计划、地址、凭证、开关和历史，不共享一次性令牌或状态。公开更新记录为 `seed-update-2026-07-29-tool-radar-live`；本批公共、API 与后台资源版本统一为 `20260729-tool-radar-live-r1`。站点资源与原创说明图先合并到 GitHub `main`，由 Cloudflare Pages 正常自动部署；线上图片 SHA-256 门禁通过后才允许生产投递。
