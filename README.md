...
- 当前独立版本为 `1.0.1`，新建元素默认使用暖白纸张、石墨线条、hachure 填充与高 roughness 的铅笔草图风。
...
- 快速画线会合并为有界增量；只有 Worker 确认已持久化的更新才从本地队列移除，限流或断线时自动重连重传。公共画布内容默认持续保留，只能由管理员显式清空。
...
- 当前独立版本为 `1.0.1`；版本、更新日志和 AI 维护约定位于 `docs/transfer/`。
