...
- 右上角 176×44 四段壁纸开关会随 05:00／11:00／17:00／20:00 自动换档，也可手动覆盖到下一边界；整条椭圆始终只呈现当前时段，四个节点常驻，激活档显示该时段独有的实心天体，并以三个不同的氛围内容层完成可中断换档。25 个 Image2／imagegen 内容源全部由 manifest 逐项锁定，发布时机械打包为 7 张不同-cell 图集加独立边框，运行时固定 8 个图片请求。键盘／motion-off 即时，reduced 只保留不超过 140ms 的纯透明度过渡，低性能档为静态。
