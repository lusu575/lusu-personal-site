...
- 右上角 176×44 四段壁纸开关会随 05:00／11:00／17:00／20:00 自动换档，也可手动覆盖到下一边界；整条椭圆始终只呈现当前安静色场，四个 20×20 语义浮雕节点常驻，激活档显示 32×32 的时段天体，每个主题只保留一个稀疏 accent 且不做错峰。17 个 Image2／imagegen 内容源全部由 manifest 逐项锁定，morning／day／dusk 的既有 accent 仅以透明画布整数像素位移校准可见位置，内容裁切字节不变且没有新增生成调用；16 个非 frame 内容机械打包为 4 张不同-cell 图集，运行时连同独立 frame 固定为 5 个图片请求。键盘／motion-off 即时，reduced 只保留不超过 140ms 的纯透明度过渡，low／Save-Data 跳过 accent 而不影响时间切换。当前发布记录为 `seed-update-2026-08-10-wallpaper-switch-calm-redesign`，公共 token 为 `20260810-wallpaper-switch-calm-r1`；素材 token 独立保持 `20260809-wallpaper-time-switch-r4`。
