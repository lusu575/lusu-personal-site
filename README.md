...
- 右上角 176×44 四段壁纸开关会随 05:00／11:00／17:00／20:00 自动换档，也可手动覆盖到下一边界。当前版本按参考图重制为完整暖象牙陶瓷椭圆壳，内开口与外轮廓对齐，可视区用 `inset(5px 6px round 17px)` 精确裁切。四个未选节点是早晨／白天／傍晚／夜晚各自高辨识的光滑拟物图标，不再使用圆点或像素化边缘。选中轮为 36×36，top 4px，四档 x 为 4／48／92／136px；其 32×32 内部天体始终正向，独立陶瓷外圈每档滚转 140deg。morning 晨光与小云上展、day 双云横移、dusk 余晖横向展开、night 八颗星上升；位移只用可中断的 transform／opacity transition，选中轮与外圈共用 `var(--motion-window)` 和 strong ease-in-out，不用 keyframes。18 个 Image2 正式内容源机械打包为 4 张 atlas，roller 作为 `node-atlas.png` 第五格复用，只有 frame 独立，运行时固定 5 个 URL且不得单独请求 `roller.png`；键盘／motion-off 即时，reduced 移除位置运动，low／Save-Data 跳过 accent。当前发布记录为 `seed-update-2026-08-10-wallpaper-switch-ceramic-roll`，公共 token 为 `20260810-wallpaper-switch-ceramic-roll-r1`，素材 token 为 `20260810-wallpaper-time-switch-r5`。
