...
## 2026-09-08 Review 迭代

- 欢迎窗改由最近更新／移动首页欢迎入口主动打开，普通访问和文章深链不再自动弹窗；知识库支持服务端游标分页、摘要与标签搜索、原生新标签打开和去重标签。
...
- 当前独立版本为 `1.0.10`；大厅身份、入口和帮助交互已收敛。受管 Agent 通道支持真实 PNG／JPEG／WebP 上传、当前房下载与只追加图片放置，Pages 主入口只把精确 raster POST 和精确 Yjs scene POST 交给完整 Agent 鉴权。公共画板与所有密码房的新建元素继续统一使用暖白纸张、石墨线条、hachure 填充与高 roughness 的铅笔草图默认值，当前不按房型提供第二套主题。
...
- 当前独立版本为 `1.0.14`；版本、更新日志和 AI 维护约定位于 `docs/transfer/`。本轮修复草稿隔房、隐藏标签上传和请求竞态，互传协议、精确同源、登录态、CSRF、加密、私有 R2 与 24 小时生命周期均未改变。
...
- 生产远程 MCP 已由独立 `workers/site-admin-mcp/` Worker `lusu-site-admin-mcp` 部署到 `https://lusu575.com/mcp`；当前精确 version ID 为 `849d8328-87db-4ac8-819a-ce725fc06349`，内部版本 `0.3.1`，承接 100% 流量。新精确 bundle 的 OAuth metadata、未鉴权 `401 WWW-Authenticate` challenge 与非 allowlist pathname `404` 线上 smoke 已通过；Production D1 migration 已完成，但这些基础检查不替代业务生命周期验收。
...
- YouTube／Bilibili／b23.tv 外链视频管理只在精确历史 bundle `377d494b-8f90-40ad-998f-863d209e1978` 通过生产闭环；测试视频 `OpwviOTPYTU` 的发布、同载荷重放、三面回读、元数据刷新、CAS 隐藏、确认删除、最终不存在与 RFC 7009 撤销均完成，临时记录已清理。当前 `849d...` 必须重新完成该生命周期；复验前 registry 不变。远程 MCP 不读取本机路径、Base64 或视频字节；真实 R2 文件上传仍未配置。
- 浏览器游戏生产点检在 2048 的配对、动作、CAS 与暂停后暴露空闲 WebSocket 断线。Pages 游戏壳的严格 8 秒 `ping`／`pong` 心跳已经上线并核验精确字节，但 Chrome 标签在后台约 5 分钟后受到强计时器节流，真实验收仍断线。当前 `849d...` 已部署 paused `observe` 下行保活：只在已鉴权、owner／grant 绑定且持续轮询时向 browser socket 发送精确原始 `pong`，随后返回缓存快照；它不更新浏览器活跃时间、不读取 provider、不执行动作、不改变 revision，也不是服务端定时器，停止轮询不保证保活，玩家仍须亲自恢复。该逻辑已部署但未完成四游戏生产验收，registry 均未晋级。
- 历史 `fa295db6-302a-4a20-a2b1-ffe1ddafd75b` 的九工具文章验收与 `377d...` 的视频验收都只绑定各自精确 bundle；当前 `849d...` 只有 metadata／401／404 基础线上点检，适用的真实 OAuth／D1／DO／浏览器和视频生命周期仍须重跑，不能复用历史证据。能力盘点见 `docs/agent-capabilities/README.md`，外部 AI 接入步骤见 `docs/agent-capabilities/REMOTE_MCP_CONNECT.md`。
...
- 右上角 176×44 四段壁纸开关会随 05:00／11:00／17:00／20:00 自动换档，也可手动覆盖到下一真实边界；四个 44px 触摸目标、自动／手动边界与跨标签同步不变。当前 r6 使用更轻薄清晰的暖象牙陶瓷椭圆壳，可视区按实测开口裁切为 `inset(4px 4px round 18px)`；36×36 wrapper 仍位于 top 4px 和 x 4／48／92／136px，内部 32×32 天体保持正向。独立 roller 的可见外径约 33.375px、开孔 30px、单边约 1.6875px，每档物理滚转 151.072deg，累计 0／151.072／302.144／453.216deg。清晨使用低位暖橙半日、晨雾／地平线和薰衣草至杏桃色场，白天保留高位完整太阳与鲜明蓝天，两者在 20／32／176×44 实际尺寸均有独立差异守卫。morning 晨光与小云上展、day 双云横移、dusk 余晖横展、night 八星上升的四主题 accent、可中断滚动、键盘／reduced／off／low／Save-Data 契约均保留。这套开关内部动效现在覆盖每个实际可见的桌面公开路由，不再只在 Home 运行；Home 整幅壁纸与动态云层仍保持首页专属，移动 App 紧凑栏仍不显示该控件。18 个当前 Image2 内容源仍机械打包为 4 张 atlas，roller 仍是 `node-atlas.png` 第五格，运行时固定 5 个 URL且不单独请求 `roller.png`；5 个被替换的 r5 来源以完整 prompt／call／SHA 保留为 superseded 历史。当前发布记录为 `seed-update-2026-08-10-wallpaper-switch-slim-dawn`，公开 API／文章 seed／main token 为 `20260810-wallpaper-switch-route-motion-r1`，素材 manifest 发布 token 保持 `20260810-wallpaper-switch-slim-dawn-r1`，素材 token 为 `20260810-wallpaper-time-switch-r6`。
- 公开主站使用“同一业务状态、两套呈现壳”：桌面端是 Neo-XP / Pixel Glass OS，移动端是带 App Home 和 safe-area 适配的虚拟手机系统；手机壳已移除重复的时间与 `LUSU OS` 状态行。真实毛玻璃 Dock 在 Home 与栏目 App 内持续悬浮，保留 Home、知识库、视频、工具、游戏、聊天室六个高频入口；关于仍从 Home 图标进入，杂谈已从移动壳下线，手机 `#blog` 会回到知识库。Dock 可横向滑动、切换并收起，账号和语言操作仍只在 Home。
...
- 远程公开读取注册层／独立无 OAuth 目标：`workers/site-mcp/`
- 生产站长 OAuth 远程 MCP Worker：`workers/site-admin-mcp/`
- 外部 AI 连接说明：`docs/agent-capabilities/REMOTE_MCP_CONNECT.md`
