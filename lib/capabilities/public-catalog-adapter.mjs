...
const AGENT_GAME_PROFILES = Object.freeze({
  ...
  "life-restart": Object.freeze({
    localSession: true,
    browserBridge: true,
    browserPairing: false,
    surface: "integrated",
    contentLanguages: Object.freeze(["zh"])
  }),
  "a-dark-room": Object.freeze({
    localSession: false,
    browserBridge: true,
    browserPairing: false,
    surface: "integrated",
...
export function listPublicTools({ lang = "zh" } = {}) {
  ...
  const tools = entries.filter((entry) => entry?.toolId !== undefined && entry.publicCatalog !== false).map((entry) => {
...
export function projectPublicGameCatalog(payload, { lang = "zh", agentOnly = false } = {}) {
  ...
  const games = payload.games.map((entry) => {
    ...
    seenIds.add(game.id);
    return game;
  }).filter((game) => !normalizedAgentOnly || game.agent.localSession || game.agent.browserPairing);
...
function projectGame(entry, lang) {
  ...
  return {
    ...
    agent: {
      ...agentProfile,
      contentLanguages: [...agentProfile.contentLanguages]
    }
  };
}
