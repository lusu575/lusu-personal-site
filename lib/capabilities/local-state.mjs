...
const DEFAULT_SITE_BASE_URL = "https://lusu575.com";
...
export function resolveSiteAuthContext(options = {}) {
  const env = options.env || process.env;
  const credential = options.credential && typeof options.credential === "object"
    ? options.credential
    : null;
  const explicitBaseUrl = String(options.baseUrl || "").trim();
  const environmentBaseUrl = String(env.LUSU_BASE_URL || "").trim();
  const credentialBaseUrl = tryNormalizeSiteBaseUrl(credential?.baseUrl);
  const baseUrl = explicitBaseUrl
    ? normalizeSiteBaseUrl(explicitBaseUrl)
    : environmentBaseUrl
      ? normalizeSiteBaseUrl(environmentBaseUrl)
      : credentialBaseUrl || normalizeSiteBaseUrl(options.defaultBaseUrl || DEFAULT_SITE_BASE_URL);
  const credentialMatchesOrigin = Boolean(
    credential
    && credentialBaseUrl
    && new URL(credentialBaseUrl).origin === new URL(baseUrl).origin
  );
  if (options.resolveAccessToken === false) {
    return Object.freeze({ baseUrl, accessToken: "", tokenSource: "none", credentialMatchesOrigin });
  }
  const explicitTokenProvided = options.explicitAccessTokenProvided === true;
  const environmentToken = String(env.LUSU_ACCESS_TOKEN || "").trim();
  let accessToken = "";
  let tokenSource = "none";
  if (explicitTokenProvided) {
    accessToken = normalizeSiteAccessToken(options.explicitAccessToken);
    tokenSource = accessToken ? "explicit" : "none";
  } else if (environmentToken) {
    accessToken = normalizeSiteAccessToken(environmentToken);
    tokenSource = "explicit";
  } else if (credentialMatchesOrigin) {
    accessToken = normalizeSiteAccessToken(credential.accessToken);
    tokenSource = accessToken ? "credential" : "none";
  }
  return Object.freeze({ baseUrl, accessToken, tokenSource, credentialMatchesOrigin });
}
...
function normalizeSiteBaseUrl(value) {
  ...
  try {
    ...
    throw new LocalStateError("The site base URL is invalid.", "AUTH_BASE_URL_INVALID", { cause: error });
  }
  if (!/^https?:$/.test(url.protocol) || url.username || url.password || url.origin === "null") {
    throw new LocalStateError("The site base URL is invalid.", "AUTH_BASE_URL_INVALID");
  }
  url.pathname = "/";
  url.search = "";
  url.hash = "";
...
function tryNormalizeSiteBaseUrl(value) {
  try {
    return normalizeSiteBaseUrl(value);
  } catch {
    return "";
  }
}

function normalizeSiteAccessToken(value) {
  const token = String(value || "").trim();
  if (!token) return "";
  if (/\s/.test(token) || token.length > 4096) {
    throw new LocalStateError("The explicit access token is invalid.", "AUTH_TOKEN_INVALID");
  }
  return token;
}
