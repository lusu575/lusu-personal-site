...
- 人工补发在稿件 `reportDate` 对应的北京时间当天 07:00（含）至次日 00:00（不含）开放。08:00 不再是截止线；跨日后仍必须停止，不能把昨天稿件冒充今天稿件。
...
普通自动任务与人工补发都允许在同一报告日 07:00 至次日 00:00 完成投递；08:00 不再触发失败。人工入口仍只用于站长明确授权的恢复任务，并保留日期与稿件 SHA-256 双确认。
