...
- 图片在契约上允许因版权或质量原因省略，但编辑目标是每个工具一张、最多两张；一旦使用，必须进入下方的视觉任务卡、成组关系、三秒测试、三语图注与失败关闭流程，不能为了填版面随意截图。
...
3. 按“本站原创可复现演示 → 明确许可的官方素材 → 本站原创说明图 → 无图”获取素材；官网可公开访问不等于允许转载。
4. 围绕完整语义区块裁切，确保关键标题、输入／动作、输出／结果不被遮挡或截断，并写表达同一结论的三语 alt 与图注。
5. 脱离正文执行三秒测试，再检查版权、隐私、清晰度、稳定性和项目本地路径。
6. 不合格就重截、改成原创说明图或省略。图片可选，但一旦进入运行记录或正文，任何视觉闸门失败都必须停止校验和投递。
7. 有图的正式期先把本期资产随 `main` 通过 GitHub → Cloudflare Pages 正常路径部署；生产投递器会从线上逐张读取并核对精确 SHA-256，图片未上线或字节不一致时不会创建文章。

首期试稿中的官网截图只用于本地人工审稿和讨论取景方法，不代表转载授权。正式周更必须重新取得合格素材与权利依据，不能把试稿截图直接当作发布资产。
...
- `prepare-first-edition-2026-07-28.mjs`：只用于首期正式稿的可审计一次性构建器，不得作为后续周更生成器
- `visuals/first-edition-2026-07-28.mjs`：只用于复现首期 7 张语言中立说明图，后续周更必须按当期视觉任务重新制图
...
令牌只从进程环境或被 Git 忽略的根 `.dev.vars` 读取，变量名为 `TOOL_RADAR_TOKEN`。目录快照放在已被 Horizon 子项目忽略的 `自动新闻/data/mcp-runs/`，不得提交令牌、抓取缓存或临时素材。有图的期次还必须先完成正常站点部署；投递器会把线上图片字节与运行记录 SHA-256 对齐后才 POST，避免公开文章引用 404 或旧缓存图片。
