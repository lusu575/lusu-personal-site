...
- `horizon.config.json`：本站 AI 新闻源配置，不含密钥。TechCrunch AI、VentureBeat AI、Ars Technica AI、雷峰网和 36氪属于可选补充；单源失败不阻断。Reddit、Hacker News 等社区源只用于早期发现，不抓取评论串；它们返回的候选仍须处置，正式新闻和时间资格必须回到规范原帖、可靠媒体或其他一手来源核验。Tibo 由 `discovery-queries.json` 中的 required 聚焦查询覆盖。
  ...
  - Google News 的 required 查询必须按发布方拆分：一条查询最多出现一个 `site:`，禁止 `OR site:` 多站点组合。每个官方或可靠媒体来源独立 required + must-review，多站点宽父查询只能 supplemental；改动后先按目标连续窗口执行 max-plus-one 实探。
  - query provenance 只读取原始 Google News topic items，或公开 X profile 显式 `queryId` 经当次权威查询目录对齐后的结果；公开 X／RSS 等直源的 `mustReviewSourceIds` 与 source review lane 由 direct-source provenance 独立保留。公开 X 抓取器不得自定 required／must-review；Tibo 的 required query 要保留必审归属，supplemental query 不能因同 URL 或直源权限被提升。
