...
The production Worker `lusu-site-admin-mcp` is deployed. The exact current version is `849d8328-87db-4ac8-819a-ce725fc06349`, internal version `0.3.1`, with 100% of production traffic, and the canonical resource is live at `https://lusu575.com/mcp`. The production `OAUTH_KV` namespace is bound, and the Production D1 migration is complete. Exact-bundle online smoke checks have passed for protected-resource and authorization-server metadata, the unauthenticated `401 WWW-Authenticate` challenge, and a `404` on a non-allowlisted path. These checks prove deployment and the base routing/auth boundary; they do not constitute DCR, browser OAuth, article, video, or game lifecycle acceptance for this bundle.

Historical version `fa295db6-302a-4a20-a2b1-ffe1ddafd75b` retains the accepted nine-tool article lifecycle. Historical version `377d494b-8f90-40ad-998f-863d209e1978` completed external-video production acceptance with `OpwviOTPYTU`: publish, same-`operationId` replay, owner/public MCP and public HTTP readback, metadata refresh, CAS hide, confirmed delete, final absence, and RFC 7009 revocation passed, and the temporary record was removed. Neither result transfers to current `849d...`; every new production Worker must repeat the applicable real-browser OAuth and lifecycle.

On 2026-08-11, internal candidate `0.4.0` was temporarily deployed as exact version `9b0bd726-2c15-414c-bdff-fc5179b4e003`. DCR, PKCE, owner OAuth, and an exact 23-tool discovery passed, but a link-only YouTube `video_publish` failed closed with `VIDEO_METADATA_TITLE_UNAVAILABLE` before any video, category, receipt, or audit write. The temporary grant was revoked and production was rolled back to `849d8328-87db-4ac8-819a-ce725fc06349` at 100% traffic. The repository candidate now combines bounded official YouTube oEmbed and watch-page requests in parallel: oEmbed wins for title/author/thumbnail, while the page enriches description/published time and can supply a fallback title. That revision is neither deployed nor production-accepted. Do not promote registry availability or repeat OAuth acceptance until the owner explicitly authorizes a new run.
...
Browser-game tools are `game_browser_pair`, `game_browser_observe`, `game_browser_actions`, `game_browser_act`, `game_browser_pause`, and `game_browser_close`. They require the non-default `games:play` scope, an active grant, a current administrator recheck, an explicit one-time owner/client-bound pairing code, and revision CAS. The relay accepts only opaque `actionId` values offered by the current semantic snapshot; selectors, scripts, raw keys, coordinates, URLs, screenshots, and DOM access are outside the protocol. Only one command may be pending, and the browser player retains visible lock, pause, take-back, close, and disconnect-unlock controls. Production 2048 smoke exposed an idle disconnect after pause. The exact eight-second Pages `ping`/`pong` heartbeat is live and its bytes are verified, but a real Chrome tab still disconnected after roughly five minutes of background timer throttling. Current production Worker `849d...` contains the paused-observe downlink fix, but all four game lifecycles remain unaccepted on this exact bundle.

The deployed `0.3.1` change affects only authenticated, owner/grant-bound `game_browser_observe` while the relay is `paused`: it requires an existing browser socket, sends exact raw text `pong` to that socket, marks the relay disconnected on absence/send failure, then only touches `lastControllerAt`, persists relay state, and returns the cached paused snapshot. It does not update `lastBrowserAt`, read the provider, create an observation, execute an action, change revision, add a protocol message type, or resume the AI. Keepalive exists only while the controller continues paused observe calls: the production acceptance helper uses a 1.5–10 second interval and the client contract must not exceed 20 seconds. There is no server timer or promise of permanent keepalive after polling stops; only browser `user_resume` can resume control. Deployment to exact version `849d...` is complete, but game lifecycle acceptance and registry promotion are not.
...
Public `videos_list` and `video_get` reads plus owner-only `video_manage_list`, `video_manage_get`, `video_publish`, `video_update`, `video_refresh_metadata`, and `video_delete` are deployed in the current Worker. Their full lifecycle acceptance belongs only to historical version `377d...`; current `849d...` and every future candidate must repeat it before availability promotion. Phase one handles only YouTube, Bilibili, and b23.tv external-link records. Publish/update/delete use active-admin rechecks, audit records, durable `operationId` receipts, and canonical payload hashes; update/delete require `expectedUpdatedAt`, while delete also requires literal `confirm: true`. The remote MCP never reads local paths, Base64, raw video bytes, or files from the AI client's machine. True hosted upload is not configured; a future upload phase requires a separate private R2 binary data plane with bounded multipart, quotas, scanning, commit, abort, expiry, and orphan cleanup.
...
Validate locally and complete the dry run before any reviewed redeploy. Run the remote migration only when the release actually contains a new approved Production D1 change; migrations required by current version `849d8328-87db-4ac8-819a-ce725fc06349` have already completed. The Pages heartbeat and the Worker paused-observe downlink are both deployed, but their deployment must not be reported as four-game lifecycle acceptance or registry promotion.
